...
```sh
bun add @xurunxin/morpheus-protocol@0.2.0 @xurunxin/morpheus-sdk@0.2.0
```
...
`verify` 覆盖格式、lint、类型、协议测试、边界与依赖 locator 检查、候选内容摘要、打包内容及空目录消费者安装。根构建通过 TypeScript paths/project references 协作，不使用 Bun workspace linking；SDK 的发布 manifest 仍精确依赖 Protocol `0.2.0`。
