...
assert(rootManifest.engines?.bun === ">=1.4.0", "根仓要求 Bun >=1.4.0");
assert(Bun.semver.satisfies(Bun.version, ">=1.4.0"), "运行时要求 Bun >=1.4.0");
assert(
...
for (const [name, value] of [
  ...
  assert(value.engines?.bun === ">=1.4.0", `${name} 要求 Bun >=1.4.0`);
