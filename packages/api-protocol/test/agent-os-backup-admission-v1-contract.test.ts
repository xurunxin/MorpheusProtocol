...
describe("agent-os backup admission application v1", () => {
  test("creates deterministic frozen bytes bound only to policy and checkpoint digests", () => {
    ...
    expect(application).toEqual({
      ...
      applicationDigest: "sha256:3d7876437b4e693181eabad8e1954f37515dcab98e1c2cd428f235a2277998a7",
    });
    expect(parseAgentOsBackupAdmissionApplicationV1(application)).toEqual(application);
    const unsigned = {
    ...
    expect(createAgentOsBackupAdmissionApplicationRefV1(unsigned)).toBe(application.applicationRef);
  ...
  test("rejects unknown, missing, path-bearing, malformed and self-drifting applications", () => {
    ...
    const invalid: unknown[] = [
      ...
      Object.fromEntries(Object.entries(application).filter(([key]) => key !== "policyDigest")),
    ...
    for (const value of invalid)
      expect(() => parseAgentOsBackupAdmissionApplicationV1(value)).toThrow(AgentOsV1ContractError);
    expect(() =>
      createAgentOsBackupAdmissionApplicationDigestV1({ ...application, credential: "raw" })
