...
function contractFixture(): AgentOsV1Contract {
  const unsignedPackage = {
    ...
    signer: { keyId: "key.effect-demo", subject: "builder.demo", algorithm: "ed25519" as const },
    trust: { domain: "trust.demo", state: "trusted" as const },
    revocation: { generation: 1, state: "active" as const },
    disabled: false as const,
    transport: { kind: "provider-adapter" as const, reference: "provider.effect-demo" },
  ...
  const agentDefinition = {
    ...
    skills: [{ id: "skill.effect-demo", packageDigest: capabilityPackage.digest }],
    tools: [{ id: "provider.effect-demo", packageDigest: capabilityPackage.digest }],
    securityPolicy: { ref: "policy:effect-demo" as const, digest: digest("policy.effect-demo") },
  };
  ...
  return {
    ...
    hostProfile: {
      ...
      providerCeiling: { ref: "ceiling:provider", digest: digest("ceiling.provider") },
      workspaceCeiling: { ref: "ceiling:workspace", digest: digest("ceiling.workspace") },
      storageCeiling: { ref: "ceiling:storage", digest: digest("ceiling.storage") },
      networkCeiling: { ref: "ceiling:network", digest: digest("ceiling.network") },
      lifecycleCeiling: { ref: "ceiling:lifecycle", digest: digest("ceiling.lifecycle") },
    },
    agentDeployment: {
      ...
      desiredReplicas: 1,
      placementPolicy: { ref: "placement:effect-demo", digest: digest("placement.effect-demo") },
      bindings: [],
    },
    executionInstance: {
...
function authorityFixture(): AgentOsEffectAuthorityBindingV1 {
  ...
  if (lease.kind !== "remote") throw new Error("fixture requires a remote lease");
...
function requestFixture(intent = intentFixture()): AgentOsEffectPermitRequestV1 {
  return {
    schemaVersion: "agent-os-effect/v1",
...
function permitFixture(
  ...
  return { ...unsigned, permitDigest: createAgentOsEffectPermitDigestV1(unsigned) };
}
...
function receiptFixture(
  ...
  return { ...unsigned, receiptDigest: createAgentOsEffectDispatchReceiptDigestV1(unsigned) };
}
...
describe("Agent OS effect/v1 protocol boundary", () => {
  ...
  test("recognizes all five closed adapter kinds", () => {
    for (const adapterKind of ["provider", "mcp", "skill", "plugin", "tool"] as const) {
      expect(parseAgentOsEffectIntentV1({ ...intentFixture(), adapterKind }).adapterKind).toBe(
        adapterKind
      );
    }
  });
  ...
  test("pins an intent to the accepted ExecutionGrant, SessionGrant, RemoteLease, Claim and package", () => {
    ...
    expect(() => assertAgentOsEffectIntentAuthorityV1({ contract, claim, intent })).not.toThrow();
    expect(() =>
      assertAgentOsEffectIntentAuthorityV1({
        contract,
        claim,
        intent: { ...intent, authority: { ...intent.authority, claimFence: 6 } },
      })
    ...
    expect(() =>
      assertAgentOsEffectIntentAuthorityV1({
        ...
        intent: {
          ...intent,
          authority: { ...intent.authority, definitionDigest: digest("stale.definition") },
        },
      })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      assertAgentOsEffectIntentAuthorityV1({
        ...
        intent: {
          ...intent,
          authority: { ...intent.authority, rotationGeneration: "rotation:stale" },
        },
      })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      assertAgentOsEffectIntentAuthorityV1({
        ...
        intent: {
          ...intent,
          capability: { ...intent.capability, packageDigest: digest("wrong.package") },
        },
      })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      assertAgentOsEffectIntentAuthorityV1({
        ...
        intent: {
          ...
          capability: { ...intent.capability, capabilityId: "provider.undeclared" },
        },
      })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
  ...
  test("allows only monotonic Permit Request narrowing", () => {
    ...
    expect(() =>
      assertAgentOsEffectPermitRequestRelationshipV1({ contract, claim, intent, request })
    ).not.toThrow();
    expect(() =>
      assertAgentOsEffectPermitRequestRelationshipV1({
        contract,
    ...
    expect(() =>
      assertAgentOsEffectPermitRequestRelationshipV1({
        ...
        claim,
        intent,
        request: { ...request, requestSchemaDigest: digest("stale.request-schema") },
      })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      assertAgentOsEffectPermitRequestRelationshipV1({
        ...
        claim,
        intent,
        request: { ...request, responseSchemaDigest: digest("stale.response-schema") },
      })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      assertAgentOsEffectPermitRequestRelationshipV1({
  ...
  test("allows only Control to issue a request- and intent-bound permit", () => {
    ...
    expect(() =>
      assertAgentOsEffectPermitRelationshipV1({ contract, claim, intent, request, permit })
    ).not.toThrow();
    expect(() => parseAgentOsEffectPermitV1({ ...permit, issuerKind: "worker" })).toThrow(
      AgentOsV1ContractError
    );
    expect(() =>
      parseAgentOsEffectPermitV1({ ...permit, permitDigest: digest("tampered.permit") })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      parseAgentOsEffectPermitV1({ ...permit, audience: ["host.demo", "worker.other"] })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      assertAgentOsEffectPermitRelationshipV1({
        contract,
  ...
  test("binds succeeded, failed and unknown receipts to adapter identity, usage and time", () => {
    ...
    expect(() =>
      assertAgentOsEffectDispatchReceiptRelationshipV1({ intent, permit, receipt })
    ).not.toThrow();
    const parsed = parseAgentOsEffectDispatchReceiptV1(receipt);
    expect(Object.isFrozen(parsed)).toBe(true);
    expect(Object.isFrozen(parsed.usage)).toBe(true);
    expect(
      parseAgentOsEffectDispatchReceiptV1(receiptFixture(intent, permit, "succeeded")).disposition
    ).toBe("succeeded");
    expect(
      parseAgentOsEffectDispatchReceiptV1(receiptFixture(intent, permit, "failed")).disposition
    ).toBe("failed");
    expect(() =>
      parseAgentOsEffectDispatchReceiptV1({
        ...receipt,
    ...
    const driftedPermitUnsigneds: AgentOsEffectPermitUnsignedV1[] = [
      ...
      {
        ...permitUnsigned,
        capability: { ...permit.capability, packageDigest: digest("drifted.package") },
      },
      { ...permitUnsigned, responseSchemaDigest: digest("drifted.response-schema") },
  ...
  test("limits unknown-effect recovery to explicit terminal resolutions with bound evidence", () => {
    ...
    expect(createAgentOsUnknownEffectRecoveryDecisionDigestV1(decision)).toMatch(
      /^sha256:[0-9a-f]{64}$/u
    );
    expect(() =>
      parseAgentOsUnknownEffectRecoveryDecisionV1({ ...decision, resolution: "retry" })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      parseAgentOsUnknownEffectRecoveryDecisionV1({ ...decision, redispatch: true })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      parseAgentOsUnknownEffectRecoveryDecisionV1({ ...decision, replay: true })
    ).toThrow(AgentOsV1ContractError);
    expect(() => parseAgentOsUnknownEffectRecoveryDecisionV1({ ...decision, revision: 0 })).toThrow(
      AgentOsV1ContractError
    );
    expect(() =>
      parseAgentOsUnknownEffectRecoveryDecisionV1({
  ...
  test("rejects unknown credentials, paths, noncanonical values and collection limits", () => {
    ...
    expect(() => parseAgentOsEffectIntentV1({ ...intent, token: "raw-secret" })).toThrow(
      AgentOsV1ContractError
    );
    expect(() => parseAgentOsEffectPermitRequestV1({ ...request, headers: {} })).toThrow(
      AgentOsV1ContractError
    );
    expect(() =>
      parseAgentOsEffectIntentV1({ ...intent, endpoint: "https://example.invalid" })
    ).toThrow(AgentOsV1ContractError);
    expect(() => parseAgentOsEffectIntentV1({ ...intent, path: "C:\\private" })).toThrow(
      AgentOsV1ContractError
    );
    expect(() =>
      parseAgentOsEffectPermitRequestV1({
    ...
    expect(() => parseAgentOsEffectIntentV1({ ...intent, adapterId: "C:\\private\\tool" })).toThrow(
      AgentOsV1ContractError
    );
    expect(() =>
      parseAgentOsEffectIntentV1({ ...intent, createdAt: "2026-08-06T00:00:05Z" })
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      parseAgentOsEffectIntentV1({ ...intent, scope: Array(33).fill("scope.demo") })
    ).toThrow(AgentOsV1ContractError);
    expect(() => parseAgentOsEffectIntentV1({ ...intent, scope: sparseScope })).toThrow(
      AgentOsV1ContractError
    );
    expect(() =>
      parseAgentOsEffectIntentV1({ ...intent, scope: ["model.invoke", "model.invoke"] })
    ).toThrow(AgentOsV1ContractError);
    expect(() => parseAgentOsEffectIntentV1(accessorIntent)).toThrow(AgentOsV1ContractError);
    expect(() => parseAgentOsEffectIntentV1(symbolIntent)).toThrow(AgentOsV1ContractError);
    expect(() =>
      parseAgentOsUnknownEffectRecoveryDecisionV1({ ...decision, reason: ` ${decision.reason}` })
    ).toThrow(AgentOsV1ContractError);
    for (const reason of [
      "password=hunter2",
      "api_key:demo-value",
      ...
      expect(() => parseAgentOsUnknownEffectRecoveryDecisionV1({ ...decision, reason })).toThrow(
        AgentOsV1ContractError
      );
    }
  });
  ...
  test("exposes all strict DTO parsers and canonical digest helpers", () => {
    ...
    const receipt = parseAgentOsEffectDispatchReceiptV1(receiptFixture(intent, permit));
    const decision = parseAgentOsUnknownEffectRecoveryDecisionV1(
      decisionFixture(intent, permit, receipt)
    );

    expect(createAgentOsEffectIntentDigestV1(intent)).toMatch(/^sha256:[0-9a-f]{64}$/u);
    expect(createAgentOsEffectPermitRequestDigestV1(request)).toMatch(/^sha256:[0-9a-f]{64}$/u);
    expect(createAgentOsEffectPermitDigestV1(permit)).toMatch(/^sha256:[0-9a-f]{64}$/u);
    expect(createAgentOsEffectDispatchReceiptDigestV1(receipt)).toMatch(/^sha256:[0-9a-f]{64}$/u);
    expect(createAgentOsUnknownEffectRecoveryDecisionDigestV1(decision)).toMatch(
      /^sha256:[0-9a-f]{64}$/u
    );
