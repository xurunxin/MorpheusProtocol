...
const PROJECTION_DIGEST = "sha256:d9a660bff93b1ab0a77746803ad38721ec65ba318c0439d0f89054aa49ba2ded";
...
async function expectAsyncCode(code: string, callback: () => Promise<unknown>): Promise<void> {
  try {
    await callback();
    throw new Error("expected contract error");
  } catch (error) {
...
describe("provider.read response and digest contract", () => {
  test("parses a metadata-only snapshot and recursively freezes it", () => {
    const parsed = parseAgentOsV1ProviderProjectionReadResponse(snapshotResponse());
  ...
  test("parses not-modified and snapshot-required without provider metadata", () => {
    expect(parseAgentOsV1ProviderProjectionReadResponse(notModifiedResponse())).toEqual(
      notModifiedResponse()
    );
    expect(parseAgentOsV1ProviderProjectionReadResponse(snapshotRequiredResponse())).toEqual(
  ...
  test("rejects forged projection digests, provider-use fields, and invalid dispositions", () => {
    ...
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadResponse(
        snapshotResponse({ projection: { ...PROJECTION, credential: "secret" } })
      )
    );
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadResponse(
...
describe("provider.read binding and correlation", () => {
  ...
  test("classifies exact replay and independent identity while rejecting drift", () => {
    ...
    const independent = createAgentOsV1ProviderProjectionRequestBinding(envelope("request.other"));
    expect(assertAgentOsV1ProviderProjectionRequestBindingCompatible(first, replay)).toBe("replay");
    expect(assertAgentOsV1ProviderProjectionRequestBindingCompatible(first, independent)).toBe(
      "independent"
    );
  ...
  test("correlates target and cursor semantics without trusting response metadata", () => {
    const nullCursor = createAgentOsV1ProviderProjectionRequestBinding(envelope());
    const cursorBinding = createAgentOsV1ProviderProjectionRequestBinding(
      envelope("request.cursor", request("cursor.same"))
    );
    expect(
      assertAgentOsV1ProviderProjectionResponseCorrelated(snapshotResponse(), nullCursor)
    ).toEqual(snapshotResponse());
    expectCode("METADATA_CORRUPT", () =>
      assertAgentOsV1ProviderProjectionResponseCorrelated(notModifiedResponse(), nullCursor)
    );
    expectCode("METADATA_CORRUPT", () =>
      assertAgentOsV1ProviderProjectionResponseCorrelated(
    ...
    expectCode("METADATA_CORRUPT", () =>
      assertAgentOsV1ProviderProjectionResponseCorrelated(
        snapshotResponse({ target: { sessionId: "session.other", storeGeneration: 3 } }),
        nullCursor
      )
    );
    expectCode("METADATA_CORRUPT", () =>
      assertAgentOsV1ProviderProjectionResponseCorrelated(snapshotRequiredResponse(), nullCursor)
    );
  });
});
...
describe("provider.read owner seam", () => {
  ...
  test("requires fresh owner admission before a metadata read", async () => {
    const fixture = owner();
    const result = await fixture.client.request(envelope(), { freshAdmission: true });
    expect(result).toEqual({ disposition: "accepted", response: snapshotResponse() });
  ...
  test("returns replay-required without owner callbacks and re-admits on explicit retry", async () => {
    const fixture = owner();
    const binding = createAgentOsV1ProviderProjectionRequestBinding(envelope());
    const replay = await fixture.client.request(envelope(), { existingBinding: binding });
    expect(replay).toEqual({ disposition: "replay_requires_fresh_admission", binding });
    expect(fixture.counts()).toEqual({ admissions: 0, reads: 0 });
    await fixture.client.request(envelope(), { existingBinding: binding, freshAdmission: true });
    expect(fixture.counts()).toEqual({ admissions: 1, reads: 1 });
  });

  test("fails closed for ambiguous different-ID recovery before owner callbacks", async () => {
    const fixture = owner();
    const existing = createAgentOsV1ProviderProjectionRequestBinding(envelope());
  ...
  test("rejects same-ID target or cursor drift before owner callbacks", async () => {
    ...
    await expectAsyncCode("IDEMPOTENCY_CONFLICT", () =>
      fixture.client.request(envelope("request.demo", request("cursor.drift")), {
        existingBinding: existing,
        freshAdmission: true,
      })
    );
  ...
  test("maps raw, custom, and proxy owner failures to redacted OWNER_UNAVAILABLE", async () => {
    ...
    const custom = createAgentOsV1ProviderProjectionReferenceClient({
      admit: () => Promise.reject({ code: "OWNER_UNAVAILABLE", message: "raw custom detail" }),
      read: () => snapshotResponse(),
    });
    await expectAsyncCode("OWNER_UNAVAILABLE", () =>
  ...
  test("preserves only the code of an exact typed owner error", async () => {
    const ownerError = createAgentOsV1ProviderProjectionReferenceClient({
      admit: () =>
        Promise.reject(
          new AgentOsV1ProviderProjectionContractError("TARGET_NOT_CURRENT", "raw owner detail")
        ),
      read: () => snapshotResponse(),
    });
  ...
  test("redacts a mutated branded owner error code", async () => {
    const ownerError = new AgentOsV1ProviderProjectionContractError("TARGET_NOT_CURRENT");
    try {
...
describe("provider.read hostile input and captured intrinsics", () => {
  test("rejects accessors, symbols, prototypes, cycles, shared references, sparse arrays, and unknown authority fields", () => {
    ...
    expectCode("INPUT_INVALID", () => parseAgentOsV1ProviderProjectionReadRequest(accessor));
    expect(getterCalls).toBe(0);

    const symbolValue = { ...request(), [Symbol("authority")]: "tenant.demo" };
    expectCode("INPUT_INVALID", () => parseAgentOsV1ProviderProjectionReadRequest(symbolValue));
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadRequest(Object.assign(Object.create(null), request()))
    );
    const cycle: Record<string, unknown> = { ...request() };
    cycle.target = cycle;
    expectCode("INPUT_INVALID", () => parseAgentOsV1ProviderProjectionReadRequest(cycle));
    ...
    sparse.length = 1;
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadRequest({ ...request(), cursor: sparse })
    );
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadRequest({ ...request(), tenantId: "tenant.demo" })
    );
  });
  ...
  test("rejects transparent proxy, depth, width, UTF-8, and oversized cursor input", () => {
    ...
    expectCode("INPUT_INVALID", () => parseAgentOsV1ProviderProjectionReadRequest(proxy));

    let nested: Record<string, unknown> = { value: "x" };
    for (let index = 0; index < 10; index += 1) nested = { value: nested };
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadRequest({ ...request(), target: nested })
    );

    const width: Record<string, unknown> = { ...request() };
    for (let index = 0; index < 40; index += 1) width[`unknown${index}`] = index;
    expectCode("INPUT_INVALID", () => parseAgentOsV1ProviderProjectionReadRequest(width));
    expectCode("INPUT_INVALID", () =>
      parseAgentOsV1ProviderProjectionReadRequest({ ...request(), cursor: "😀".repeat(400) })
    );
  });
  ...
  test("does not execute hostile factory or signal option accessors", async () => {
    ...
    await expectAsyncCode("INPUT_INVALID", () => fixture.request(envelope(), options));
  ...
  test("fails closed if the global structuredClone intrinsic is replaced", () => {
    ...
    try {
      ...
      expectCode("INPUT_INVALID", () => parseAgentOsV1ProviderProjectionReadRequest(request()));
    } finally {
  ...
  test("uses captured AbortSignal intrinsics for pre-abort owner0", async () => {
    ...
    try {
      Reflect.apply = (() => false) as typeof Reflect.apply;
      await expectAbort(() =>
        client.request(envelope(), { freshAdmission: true, signal: controller.signal })
      );
  ...
  test("uses captured TextEncoder byte counts for oversized UTF-8 cursors", () => {
    ...
    try {
      globalThis.TextEncoder = ZeroByteTextEncoder as typeof globalThis.TextEncoder;
      expectCode("INPUT_INVALID", () =>
        parseAgentOsV1ProviderProjectionReadRequest({
          ...request(),
