...
describe("agent-os effect budget admission application v1", () => {
  test("重现 DAR-565 的确定性 bytes、ref 与标准 SHA-256", () => {
    ...
    expect(createAgentOsEffectBudgetAdmissionApplicationDigestV1(unsigned)).toBe(expectedDigest);
    expect(createAgentOsEffectBudgetAdmissionApplicationRefV1(unsigned)).toBe(
      application.applicationRef
    );
    expect(parseAgentOsEffectBudgetAdmissionApplicationV1(application)).toEqual(application);
    expect(Object.isFrozen(application)).toBe(true);
    expect(serializeAgentOsEffectBudgetAdmissionApplicationV1(application)).toBe(
  ...
  test("未知、缺失、非法和漂移输入全部 fail closed", () => {
    ...
    const invalid: unknown[] = [
      ...
      Object.fromEntries(Object.entries(application).filter(([key]) => key !== "commandId")),
      ...
      Object.defineProperty({ ...application }, "commandId", { get: () => "command.effect-1" }),
      Object.assign({ ...application }, { [Symbol("hidden")]: true }),
    ];
    for (const value of invalid)
      expect(() => parseAgentOsEffectBudgetAdmissionApplicationV1(value)).toThrow(
        AgentOsV1ContractError
      );
  });
  ...
  test("unsigned constructors 同样拒绝额外字段和非普通对象", () => {
    expect(() =>
      createAgentOsEffectBudgetAdmissionApplicationDigestV1({ ...unsigned, rawToken: "secret" })
