...
export function parseAgentOsEffectIntentV1(input: unknown): Readonly<AgentOsEffectIntentV1> {
  ...
  return deepFreeze({
    ...
    requestSchemaDigest: digest(value.requestSchemaDigest, "effect intent requestSchemaDigest"),
    responseSchemaDigest: digest(value.responseSchemaDigest, "effect intent responseSchemaDigest"),
...
export function parseAgentOsEffectPermitRequestV1(
  ...
  return deepFreeze({
    ...
    intentDigest: digest(value.intentDigest, "effect permit request intentDigest"),
    effectId: identifier(value.effectId, "effect permit request effectId"),
    adapterKind: adapterKind(value.adapterKind, "effect permit request adapterKind"),
    ...
    authority: authorityBinding(value.authority, "effect permit request authority"),
    capability: capabilityBinding(value.capability, "effect permit request capability"),
    ...
    handlerDigest: digest(value.handlerDigest, "effect permit request handlerDigest"),
    ...
    requestedScope: identifiers(value.requestedScope, "effect permit request requestedScope"),
    requestedAt: instant(value.requestedAt, "effect permit request requestedAt"),
    notBefore,
    expiresAt,
  });
}
...
export function parseAgentOsEffectPermitV1(input: unknown): Readonly<AgentOsEffectPermitV1> {
  const value = record(input, "effect permit");
  exact(value, [...EFFECT_PERMIT_UNSIGNED_KEYS, "permitDigest"], "effect permit");
...
export function parseAgentOsEffectDispatchReceiptV1(
  ...
  exact(value, [...EFFECT_RECEIPT_UNSIGNED_KEYS, "receiptDigest"], "effect dispatch receipt");
  const unsigned = receiptUnsigned(value);
  const receiptDigest = digest(value.receiptDigest, "effect dispatch receipt receiptDigest");
  const expectedDigest = contentDigest(canonicalJson(unsigned));
  if (receiptDigest !== expectedDigest)
    fail("DRIFT_DETECTED", "effect dispatch receipt receiptDigest is self-inconsistent");
...
export function parseAgentOsUnknownEffectRecoveryDecisionV1(
  ...
  return deepFreeze({
    schemaVersion: "agent-os-effect/v1",
    decisionId: identifier(value.decisionId, "unknown effect recovery decision decisionId"),
    actorId: identifier(value.actorId, "unknown effect recovery decision actorId"),
    revision: positiveInteger(value.revision, "unknown effect recovery decision revision"),
    effectId: identifier(value.effectId, "unknown effect recovery decision effectId"),
    resolution: recoveryResolution(value.resolution),
    intentDigest: digest(value.intentDigest, "unknown effect recovery decision intentDigest"),
    permitDigest: digest(value.permitDigest, "unknown effect recovery decision permitDigest"),
    dispatchReceiptDigest: digest(
      value.dispatchReceiptDigest,
      "unknown effect recovery decision dispatchReceiptDigest"
    ),
    authority: authorityBinding(value.authority, "unknown effect recovery decision authority"),
    evidenceDigest: digest(value.evidenceDigest, "unknown effect recovery decision evidenceDigest"),
    reason: canonicalText(value.reason, "unknown effect recovery decision reason", 1_024),
    decidedAt: instant(value.decidedAt, "unknown effect recovery decision decidedAt"),
  });
}
...
export function serializeAgentOsEffectDispatchReceiptV1(input: unknown): string {
  return canonicalJson(parseAgentOsEffectDispatchReceiptV1(input));
}

export function serializeAgentOsUnknownEffectRecoveryDecisionV1(input: unknown): string {
...
export function createAgentOsEffectPermitRequestDigestV1(input: unknown): string {
...
export function createAgentOsEffectPermitDigestV1(input: unknown): string {
  ...
  if ("permitDigest" in value) return parseAgentOsEffectPermitV1(input).permitDigest;
...
export function createAgentOsEffectDispatchReceiptDigestV1(input: unknown): string {
  const value = record(input, "effect dispatch receipt digest input");
  if ("receiptDigest" in value) return parseAgentOsEffectDispatchReceiptV1(input).receiptDigest;
  exact(value, EFFECT_RECEIPT_UNSIGNED_KEYS, "effect dispatch receipt digest input");
  return contentDigest(canonicalJson(receiptUnsigned(value)));
}

export function createAgentOsUnknownEffectRecoveryDecisionDigestV1(input: unknown): string {
...
export function assertAgentOsEffectIntentAuthorityV1(
  ...
  const context = authorityContext(input.contract, input.claim, intent.authority.turnId);
  assertSame("effect intent authority pins drifted", intent.authority, context.authority);
  ...
  if (
    ...
    fail("DRIFT_DETECTED", "effect intent capability is not declared by the agent definition");
  narrow(
    intent.audience,
    context.contract.executionGrant.audience,
    "effect audience expands grant"
  );
  narrow(intent.scope, context.contract.executionGrant.scope, "effect scope expands grant");
  narrow(
    intent.scope,
    context.contract.executionGrant.sessionGrant.scope,
    "effect scope expands session grant"
  );
  narrow(intent.scope, context.lease.scope, "effect scope expands remote lease");
  narrow(
    intent.scope,
    context.contract.runSpec.capabilityScopes,
    "effect scope expands run capability scopes"
  );
  assertWithinAuthorityWindow(intent.createdAt, context, "effect intent createdAt");
}
...
export function assertAgentOsEffectPermitRequestRelationshipV1(
  ...
  if (request.intentDigest !== createAgentOsEffectIntentDigestV1(intent))
    fail("DRIFT_DETECTED", "effect permit request intentDigest is inconsistent");
  if (request.effectId !== intent.effectId)
    fail("DRIFT_DETECTED", "effect permit request effectId drifted");
  assertSame("effect permit request authority drifted", request.authority, intent.authority);
  assertSame("effect permit request capability drifted", request.capability, intent.capability);
  if (
  ...
  narrow(request.requestedScope, intent.scope, "effect permit request scope expands intent");
  if (
    request.requestedAt < intent.createdAt ||
    request.requestedAt > request.notBefore ||
    request.notBefore < intent.createdAt
  )
    fail("GRANT_EXPANSION", "effect permit request timing precedes its intent or request");
  assertWindowWithinAuthority(request.notBefore, request.expiresAt, context, "permit request");
}
...
export function assertAgentOsEffectPermitRelationshipV1(
  ...
  if (permit.issuerId !== contract.executionGrant.issuer)
    fail("DRIFT_DETECTED", "effect permit Control issuer differs from the execution grant issuer");
  if (permit.requestDigest !== createAgentOsEffectPermitRequestDigestV1(request))
  ...
  assertSame("effect permit authority drifted", permit.authority, request.authority);
  assertSame("effect permit capability drifted", permit.capability, request.capability);
  if (
  ...
  narrow(permit.audience, request.requestedAudience, "effect permit audience expands request");
  narrow(permit.scope, request.requestedScope, "effect permit scope expands request");
  narrowWindow(
    permit.notBefore,
    permit.expiresAt,
    request.notBefore,
  ...
  if (permit.issuedAt < request.notBefore || permit.issuedAt > permit.expiresAt)
    fail("DRIFT_DETECTED", "effect permit issuedAt is outside the requested validity window");
}
...
export function assertAgentOsEffectDispatchReceiptRelationshipV1(
  ...
  if (
    ...
    fail("DRIFT_DETECTED", "effect dispatch receipt digest binding is inconsistent");
  if (
  ...
  assertSame("effect dispatch receipt authority drifted", receipt.authority, intent.authority);
  assertSame(
  ...
  narrow(permit.audience, intent.audience, "effect permit audience expands dispatched intent");
  narrow(permit.scope, intent.scope, "effect permit scope expands dispatched intent");
  ...
  if (receipt.dispatchedAt < permit.notBefore || receipt.dispatchedAt > permit.expiresAt)
...
export function assertAgentOsUnknownEffectRecoveryDecisionRelationshipV1(
  ...
  if (receipt.disposition !== "unknown")
    fail("INVALID_VALUE", "unknown effect recovery requires an unknown dispatch receipt");
  if (
    ...
    fail("DRIFT_DETECTED", "unknown effect recovery decision digest binding is inconsistent");
  ...
  if (decision.decidedAt < receipt.completedAt)
    fail("DRIFT_DETECTED", "unknown effect recovery decision predates dispatch completion");
}
...
function authorityContext(
  ...
  readonly lease: Extract<AgentOsV1Contract["executionGrant"]["leaseBinding"], { kind: "remote" }>;
  ...
  if (grant.kind === "local" || lease.kind !== "remote")
    fail("GRANT_EXPANSION", "effect dispatch requires a remote or delegated grant and lease");
  if (
    ...
    fail("DRIFT_DETECTED", "effect claim does not pin the current execution authority");
...
function assertWindowWithinAuthority(
  ...
  const grant = context.contract.executionGrant;
  narrowWindow(notBefore, expiresAt, grant.notBefore, grant.expiresAt, `${label} expands grant`);
  narrowWindow(
    notBefore,
    expiresAt,
...
function permitUnsigned(value: Record<string, unknown>): Readonly<AgentOsEffectPermitUnsignedV1> {
  if (value.schemaVersion !== "agent-os-effect/v1")
    fail("UNSUPPORTED_VERSION", "effect permit schemaVersion must equal agent-os-effect/v1");
  ...
  return deepFreeze({
    ...
    requestSchemaDigest: digest(value.requestSchemaDigest, "effect permit requestSchemaDigest"),
    responseSchemaDigest: digest(value.responseSchemaDigest, "effect permit responseSchemaDigest"),
...
function receiptUnsigned(
  ...
  const dispatchedAt = instant(value.dispatchedAt, "effect dispatch receipt dispatchedAt");
  const completedAt = instant(value.completedAt, "effect dispatch receipt completedAt");
  if (completedAt < dispatchedAt)
    fail("DRIFT_DETECTED", "effect dispatch receipt completion predates dispatch");
  const usage = effectUsage(value.usage);
  if (disposition === "unknown" && usage.totalUnits !== 0)
    fail("DRIFT_DETECTED", "unknown effect dispatch receipt must not claim usage");
  return deepFreeze({
    schemaVersion: "agent-os-effect/v1",
    receiptId: identifier(value.receiptId, "effect dispatch receipt receiptId"),
    disposition,
    intentDigest: digest(value.intentDigest, "effect dispatch receipt intentDigest"),
    permitDigest: digest(value.permitDigest, "effect dispatch receipt permitDigest"),
    ...
    adapterKind: adapterKind(value.adapterKind, "effect dispatch receipt adapterKind"),
    ...
    requestDigest: digest(value.requestDigest, "effect dispatch receipt requestDigest"),
    responseDigest: digest(value.responseDigest, "effect dispatch receipt responseDigest"),
    authority: authorityBinding(value.authority, "effect dispatch receipt authority"),
    usage,
    dispatchedAt,
    completedAt,
  });
...
function effectUsage(input: unknown): Readonly<AgentOsEffectUsageV1> {
  ...
  exact(value, ["inputUnits", "outputUnits", "totalUnits"], "effect dispatch receipt usage");
  const inputUnits = nonNegativeInteger(value.inputUnits, "effect usage inputUnits");
  const outputUnits = nonNegativeInteger(value.outputUnits, "effect usage outputUnits");
  const totalUnits = nonNegativeInteger(value.totalUnits, "effect usage totalUnits");
  if (!Number.isSafeInteger(inputUnits + outputUnits) || totalUnits !== inputUnits + outputUnits)
    fail("DRIFT_DETECTED", "effect usage totalUnits is inconsistent");
  return deepFreeze({ inputUnits, outputUnits, totalUnits });
}

function authorityBinding(input: unknown, label: string): AgentOsEffectAuthorityBindingV1 {
  ...
  return deepFreeze({
    ...
    rotationGeneration: rotationGeneration(value.rotationGeneration, `${label} rotationGeneration`),
    ...
    authorityDomain: identifier(value.authorityDomain, `${label} authorityDomain`),
    ...
    instanceGeneration: nonNegativeInteger(value.instanceGeneration, `${label} instanceGeneration`),
    ...
    storeGeneration: positiveInteger(value.storeGeneration, `${label} storeGeneration`),
    definitionDigest: digest(value.definitionDigest, `${label} definitionDigest`),
    policyDigest: digest(value.policyDigest, `${label} policyDigest`),
    capabilityDigest: digest(value.capabilityDigest, `${label} capabilityDigest`),
    keyId: identifier(value.keyId, `${label} keyId`),
  });
}

function capabilityBinding(input: unknown, label: string): AgentOsEffectCapabilityBindingV1 {
  const value = record(input, label);
...
function effectRecord(
  ...
  if (value.schemaVersion !== "agent-os-effect/v1")
    fail("UNSUPPORTED_VERSION", `${label} schemaVersion must equal agent-os-effect/v1`);
  return value;
}
...
function identifiers(value: unknown, label: string): readonly string[] {
  ...
  if (values.length === 0 || values.length > MAX_BINDING_ITEMS)
    fail("INVALID_VALUE", `${label} must contain 1 to ${MAX_BINDING_ITEMS} items`);
  const result = values.map((item, index) => identifier(item, `${label}[${index}]`)).sort();
...
function singletonAudience(value: unknown, label: string): readonly [string] {
  ...
  if (result.length !== 1) fail("GRANT_EXPANSION", `${label} must contain exactly one principal`);
...
function qualifiedRef(value: unknown, namespace: string, label: string): `idempotency:${string}` {
  if (
    typeof value !== "string" ||
...
function leaseEpoch(value: unknown, label: string): AgentOsEffectAuthorityBindingV1["leaseEpoch"] {
...
function dispatchDisposition(value: unknown): AgentOsEffectDispatchDispositionV1 {
  if (value !== "succeeded" && value !== "failed" && value !== "unknown")
    fail("INVALID_VALUE", "effect dispatch receipt disposition is invalid");
  return value;
}

function recoveryResolution(value: unknown): AgentOsUnknownEffectRecoveryResolutionV1 {
  if (
...
function canonicalText(value: unknown, label: string, maxBytes: number): string {
  ...
  if (
    ...
    fail("INVALID_VALUE", `${label} must be canonical trimmed text within ${maxBytes} bytes`);
  return value;
}
...
function assertOrderedWindow(notBefore: string, expiresAt: string, label: string): void {
...
function assertAdapterTransport(
  ...
  const matches =
    ...
    adapter === "tool";
  if (!matches) fail("DRIFT_DETECTED", "effect adapter kind differs from capability transport");
}
...
function canonicalJson(value: unknown): string {
  ...
  fail("INVALID_VALUE", "canonical effect source contains an unsupported value");
}
