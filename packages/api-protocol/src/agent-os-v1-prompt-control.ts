...
export const AGENT_OS_V1_PROMPT_CONTROL_SCHEMA = "agent-os-prompt-control/v1" as const;
export const AGENT_OS_V1_PROMPT_STEER_OPERATION = "prompt.steer" as const;
export const AGENT_OS_V1_PROMPT_FOLLOW_UP_OPERATION = "prompt.follow-up" as const;
...
export type AgentOsV1PromptControlRequestBindingComparison = "replay" | "independent";
...
export function parseAgentOsV1PromptControlRequest(
  ...
  const value = copyPlainData(input);
  if (value.operation === AGENT_OS_V1_PROMPT_STEER_OPERATION) return parseSteerValue(value);
  if (value.operation === AGENT_OS_V1_PROMPT_FOLLOW_UP_OPERATION) return parseFollowUpValue(value);
  fail("INPUT_INVALID");
}
...
export const parseAgentOsV1PromptControlResponse = parseAgentOsV1PromptControlReceipt;
...
export function canonicalAgentOsV1PromptControlInstructionSource(input: unknown): string {
...
export function createAgentOsV1PromptControlInstructionDigest(input: unknown): string {
...
function parseSteerValue(value: Record<string, unknown>): Readonly<AgentOsV1PromptSteerRequest> {
  exact(value, ["schemaVersion", "operation", "target", "instruction", "instructionDigest"]);
  if (value.schemaVersion !== AGENT_OS_V1_PROMPT_CONTROL_SCHEMA) fail("INPUT_INVALID");
  if (value.operation !== AGENT_OS_V1_PROMPT_STEER_OPERATION) fail("INPUT_INVALID");
  ...
  if (instructionDigest !== createAgentOsV1PromptControlInstructionDigest(instruction))
    fail("INPUT_INVALID");
  return deepFreeze({
    schemaVersion: AGENT_OS_V1_PROMPT_CONTROL_SCHEMA,
...
function parseFollowUpValue(
  ...
  exact(value, ["schemaVersion", "operation", "predecessor", "instruction", "instructionDigest"]);
  if (value.schemaVersion !== AGENT_OS_V1_PROMPT_CONTROL_SCHEMA) fail("INPUT_INVALID");
  if (value.operation !== AGENT_OS_V1_PROMPT_FOLLOW_UP_OPERATION) fail("INPUT_INVALID");
...
function parseRequestIdentity(input: unknown): {
  ...
  const target =
    request.operation === AGENT_OS_V1_PROMPT_STEER_OPERATION ? request.target : request.predecessor;
...
function captureReferenceClientOptions(
  ...
  if (!isReferenceAdmit(admit) || !isReferenceDispatch(dispatch)) fail("INPUT_INVALID");
  return { admit, dispatch };
}

function isReferenceAdmit(value: unknown): value is AgentOsV1PromptControlReferenceAdmit {
  return typeof value === "function";
}

function isReferenceDispatch(value: unknown): value is AgentOsV1PromptControlReferenceDispatch {
  return typeof value === "function";
}
...
function parseTarget(value: unknown, label: string): Readonly<AgentOsV1PromptControlTarget> {
  ...
  return deepFreeze({
    ...
    runId: identifierText(target.runId, AGENT_OS_V1_PROMPT_CONTROL_LIMITS.maxRunIdUtf8Bytes),
    attemptId: identifierText(
      target.attemptId,
      AGENT_OS_V1_PROMPT_CONTROL_LIMITS.maxAttemptIdUtf8Bytes
    ),
    storeGeneration: positiveInteger(target.storeGeneration, `${label}.storeGeneration`),
  });
}

function parseInstruction(value: unknown): Readonly<AgentOsV1PromptControlInstruction> {
  ...
  const copied = messages.map((message) => {
    ...
    if (typeof item.content !== "string" || item.content.length === 0) fail("INPUT_INVALID");
    instructionBytes += utf8Length(item.content);
    if (instructionBytes > AGENT_OS_V1_PROMPT_CONTROL_LIMITS.maxInstructionBytes)
      fail("INPUT_INVALID");
...
function copyValue(input: unknown, state: CopyState, depth: number): unknown {
  ...
  if (depth >= AGENT_OS_V1_PROMPT_CONTROL_LIMITS.maxDepth) fail("INPUT_INVALID");
  ...
  const keys = Object.keys(descriptors);
  if (keys.length > AGENT_OS_V1_PROMPT_CONTROL_LIMITS.maxWidth) fail("INPUT_INVALID");
  const output: Record<string, unknown> = {};
  for (const key of keys) {
    countUtf8(key, state);
    const descriptor = descriptors[key];
...
function countUtf8(value: string, state: CopyState): void {
  ...
  if (state.totalUtf8Bytes > AGENT_OS_V1_PROMPT_CONTROL_LIMITS.maxTotalUtf8Bytes)
    fail("INPUT_INVALID");
}
