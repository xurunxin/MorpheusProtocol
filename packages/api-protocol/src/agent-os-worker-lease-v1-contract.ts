...
export function parseAgentOsWorkerLeaseV1Envelope(
  ...
  if (value.schemaVersion !== "agent-os-worker-lease/v1")
    fail("UNSUPPORTED_VERSION", "schemaVersion must equal agent-os-worker-lease/v1");
  ...
  if (deadline < requestedAt) fail("DRIFT_DETECTED", "deadline predates requestedAt");
  const payload = parsePayload(operation, value.payload);
  const payloadDigest = digest(value.payloadDigest, "payloadDigest");
  const expectedDigest = createAgentOsWorkerLeaseV1PayloadDigest(operation, payload);
  ...
  return deepFreeze({ ...canonical, envelopeDigest }) as Readonly<AgentOsWorkerLeaseV1Envelope>;
}
...
>(input: {
  ...
  const payloadDigest = createAgentOsWorkerLeaseV1PayloadDigest(input.operation, payload);
  const canonical = {
    schemaVersion: "agent-os-worker-lease/v1" as const,
    ...input,
    payload,
...
function createEnvelopeDigest(input: Readonly<Record<string, unknown>>): string {
...
function availabilityPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1AvailabilityPayload> {
  ...
  const maxActiveRuns = positiveInteger(capacity.maxActiveRuns, "maxActiveRuns");
  const activeRuns = nonNegativeInteger(capacity.activeRuns, "activeRuns");
  if (activeRuns > maxActiveRuns) fail("DRIFT_DETECTED", "activeRuns exceeds maxActiveRuns");
  if (typeof value.draining !== "boolean") fail("INVALID_VALUE", "draining must be boolean");
  return deepFreeze({
...
function claimRequestPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1ClaimRequestPayload> {
  const value = payloadRecord(input, ["commandId", "grant", "instance", "claim"]);
  const grant = parseAgentOsV1ExecutionGrant(value.grant);
  const instance = parseAgentOsV1ExecutionInstance(value.instance);
...
function claimAckPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1ClaimAckPayload> {
  const value = payloadRecord(input, ["commandId", "claim", "accepted", "rejection"]);
  if (typeof value.accepted !== "boolean") fail("INVALID_VALUE", "accepted must be boolean");
...
function renewPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1RenewPayload> {
  const value = payloadRecord(input, ["commandId", "grant", "instance", "claim"]);
  const grant = parseAgentOsV1ExecutionGrant(value.grant);
  const instance = parseAgentOsV1ExecutionInstance(value.instance);
  const claim = parseAgentOsV1ExecutionClaimBinding(value.claim);
...
function progressPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1ProgressPayload> {
  const value = payloadRecord(input, [
    "commandId",
    "claim",
    "revision",
...
function resultPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1ResultPayload> {
  const value = payloadRecord(input, [
    "commandId",
    "claim",
    "status",
  ...
  const artifactDigest =
    value.artifactDigest === null ? null : digest(value.artifactDigest, "artifactDigest");
...
function cancelPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1CancelPayload> {
...
function drainPayload(input: unknown): Readonly<AgentOsWorkerLeaseV1DrainPayload> {
  const value = payloadRecord(input, ["commandId", "mode", "reasonDigest", "deadline"]);
...
function quarantinePayload(input: unknown): Readonly<AgentOsWorkerLeaseV1QuarantinePayload> {
  const value = payloadRecord(input, ["commandId", "targetMessageId", "reason", "evidenceDigest"]);
  return deepFreeze({
    commandId: identifier(value.commandId, "commandId"),
...
function workloadIdentity(input: unknown): Readonly<AgentOsWorkerLeaseV1WorkloadIdentity> {
  const value = record(input, "workload identity");
  exact(
    value,
    ["issuer", "subject", "audience", "keyId", "rotationGeneration", "revocationGeneration"],
    "workload identity"
  );
  if (!Array.isArray(value.audience) || value.audience.length !== 1 || !(0 in value.audience))
    fail("INVALID_SHAPE", "workload identity audience must contain exactly one item");
  return deepFreeze({
    issuer: identifier(value.issuer, "identity issuer"),
    subject: identifier(value.subject, "identity subject"),
    audience: [identifier(value.audience[0], "identity audience")] as readonly [string],
    keyId: identifier(value.keyId, "identity keyId"),
    rotationGeneration: qualified(value.rotationGeneration, "rotation", "rotationGeneration"),
...
function capabilityManifest(input: unknown): Readonly<AgentOsWorkerLeaseV1CapabilityManifest> {
  const value = record(input, "capability manifest");
  exact(value, ["packageId", "packageDigest", "capabilityDigest"], "capability manifest");
  return deepFreeze({
...
function assertEnvelopePayloadPins(input: {
  ...
  if (input.operation === "worker.availability") {
    ...
    if (
      ...
      fail("DRIFT_DETECTED", "availability identity does not bind the Worker and Control audience");
    if (availability.observedAt > input.deadline)
      fail("DRIFT_DETECTED", "availability observation exceeds the envelope deadline");
    return;
  }
  if (
...
function payloadRecord(input: unknown, keys: readonly string[]): Record<string, unknown> {
...
function assertOperationSender(
  ...
  const expected =
    ...
    operation === "execution.result"
      ? "worker"
      : "control";
  if (sender !== expected) fail("DRIFT_DETECTED", `${operation} must be sent by ${expected}`);
}

function claimRejection(input: unknown): AgentOsWorkerLeaseV1ClaimRejection | null {
...
function quarantineReason(input: unknown): AgentOsWorkerLeaseV1QuarantineReason {
  if (
...
function instant(input: unknown, label: string): string {
  ...
  if (new Date(input).toISOString() !== input) fail("INVALID_VALUE", `${label} is invalid`);
  return input;
}
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) fail("INVALID_VALUE", "canonical value must be finite");
...
function fail(code: AgentOsWorkerLeaseV1ContractErrorCode, message: string): never {
