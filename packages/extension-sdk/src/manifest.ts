...
export type SupportedProviderToolcallRoute = Extract<ToolcallRoute, "wasm.exec" | "tool.exec">;
export type SupportedProviderExecutorType = Extract<ToolRegistryExecutorType, "wasm" | "tool">;
...
function validateTools(
  ...
  for (const [index, tool] of value.entries()) {
    ...
    if (isRecord(tool.executor) && isNonEmptyString(toolId)) {
      ...
      const target = isNonEmptyString(tool.executor.target) ? tool.executor.target : toolId;
    ...
    validateToolCapabilities(tool.capabilities, capabilityIds, diagnostics, `${path}.capabilities`);
...
function isExtensionProviderKind(value: unknown): value is ExtensionProviderKind {
  return (
...
function isToolRegistryExecutorType(value: unknown): value is ToolRegistryExecutorType {
  return value === "wasm" || value === "bash" || value === "tool" || value === "container";
}
...
function executorTypeMatchesRoute(
  ...
  return (type === "wasm" && route === "wasm.exec") || (type === "tool" && route === "tool.exec");
}
