...
function handshakeOffer(side: "client" | "provider", versions: readonly string[] = ["1.1"]) {
  const peerId = side === "client" ? "terminal.execution" : "kernel.execution";
  const audience = [side === "client" ? "kernel.execution" : "terminal.execution"];
  return {
    protocol: {
...
describe("SDK Prompt AbortSignal orchestration", () => {
  test("does not create inputs or dispatch when the signal is already aborted", async () => {
    ...
    } satisfies Pick<AgentOsV1CanonicalPromptReferenceClient, "start" | "cancel">;
  ...
  test("forwards caller inputs and dispatches exactly one cancel for a mid-flight abort", async () => {
    ...
    const { startResponse, cancelResponse, startArguments, cancelArguments } = CANONICAL_FIXTURE;
    ...
    await expect(running).resolves.toEqual({ kind: "cancelled", response: cancelResponse });
...
describe("SDK typed App handshake", () => {
  test("delegates strict negotiation and exposes accepted and UPDATE_REQUIRED outcomes", async () => {
    ...
    ).resolves.toEqual({ status: "UPDATE_REQUIRED", reason: "UNKNOWN_VERSION" });
  });
