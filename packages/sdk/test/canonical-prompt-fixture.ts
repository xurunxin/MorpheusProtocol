...
export function createCanonicalPromptFixture(): {
  ...
  const startPayload = parseAgentOsV1CanonicalPromptStartRequest(startBinding.payload);
  ...
  return {
    startArguments: [startEnvelope, snapshot, startPayload, NOW] satisfies PromptStartArguments,
    cancelArguments: [cancelEnvelope, snapshot, cancelPayload, NOW] satisfies PromptCancelArguments,
    startResponse: createResponse("prompt.start", "succeeded", START_REQUEST_ID),
    cancelResponse: createResponse("prompt.cancel", "cancelled", CANCEL_REQUEST_ID),
  };
}
