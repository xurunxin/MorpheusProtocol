...
describe("SDK immutable Prompt projection", () => {
  test("commits contiguous deltas without mutating the previous projection", () => {
    ...
    if (initial.kind !== "committed") throw new Error("expected committed projection");
    ...
    if (next.kind !== "committed") throw new Error("expected committed projection");
  ...
  test("drops partial deltas and requires an atomic rebuild after authority epoch drift", () => {
    ...
    expect(stale).toEqual({ kind: "rebuild-required", reason: "authority-epoch-changed" });
    ...
    if (rebuilt.kind !== "committed") throw new Error("expected committed rebuild");
  ...
  test("deduplicates identical event digests and rejects conflicting replay or a future cursor", () => {
    ...
    if (duplicate.kind !== "committed") throw new Error("expected idempotent projection");
