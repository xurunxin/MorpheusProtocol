...
export type PromptProjectionTransition =
  ...
  | Readonly<{
      ...
      reason: PromptProjectionRebuildReason;
    }>
  | Readonly<{
...
export function transitionPromptProjection(
  ...
  if (
    previous !== null &&
    page.response.snapshot.runId !== previous.snapshot.runId
  ) {
    return Object.freeze({ kind: "rebuild-required", reason: "run-changed" });
  }
  ...
  if (
    ...
    (first === undefined &&
      page.response.cursor.sequence > previous.cursor.sequence)
  ) {
...
function rebuiltState(
  page: Readonly<AgentOsV1AppProjectionPage>,
): Readonly<PromptProjectionState> {
  return Object.freeze({
    tenantId: page.tenantId,
    authorityEpoch: page.authorityEpoch,
    lifecycle: page.lifecycle,
