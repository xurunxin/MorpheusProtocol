...
import {
  ...
  AgentOsWorkerLeaseV1ContractError as PackageAgentOsWorkerLeaseV1ContractError,
...
describe("agent-os-worker-lease/v1 strict contract", () => {
  test("canonicalizes and freezes every operation in the v1 family", () => {
    const messages = [
      ...
      envelope(
        "claim.request",
        { commandId: "command.claim", grant, instance, claim },
        3,
      ),
      envelope(
        "claim.ack",
      ...
      envelope(
        "lease.renew",
        { commandId: "command.renew", grant, instance, claim },
        5,
      ),
      envelope(
        "execution.progress",
    ...
    expect(messages.every((message) => Object.isFrozen(message.payload))).toBe(
      true,
    );
    expect(new Set(messages.map((message) => message.payloadDigest)).size).toBe(
      11,
    );
    expect(
      new Set(messages.map((message) => message.envelopeDigest)).size,
    ).toBe(11);
  });
  ...
  test("is deterministic and does not mutate the immutable agent-os/v1 fixture", () => {
    ...
    expect(
      JSON.stringify(AGENT_OS_V1_IMMUTABLE_PROMPT_REFERENCE_ARTIFACT),
    ).toBe(before);
  });
  ...
  test("matches an independent SHA-256 implementation and exports equivalent parser behavior", () => {
    ...
    expect(message.payloadDigest).toBe(
      digest(
        canonical({ operation: message.operation, payload: message.payload }),
      ),
    );
    expect(envelopeDigest).toBe(digest(canonical(unsigned)));
    expect(parseFromPackage(message)).toEqual(
      parseAgentOsWorkerLeaseV1Envelope(message),
    );
    expect(
      AgentOsWorkerLeaseV1.parseAgentOsWorkerLeaseV1Envelope(message),
    ).toEqual(parseAgentOsWorkerLeaseV1Envelope(message));
    expect(() => parseFromPackage({})).toThrow(
      PackageAgentOsWorkerLeaseV1ContractError,
    );
    expect(() =>
      parseFromPackage({ ...message, envelopeDigest: digest("mismatch") }),
    ).toThrow("DIGEST_MISMATCH");
    expect(
      AgentOsWorkerLeaseV1Reference.runAgentOsWorkerLeaseV1Conformance,
    ).toBeFunction();
  });
  ...
  test("rejects unknown, secret-like and sparse fields", () => {
    ...
    expect(() =>
      parseAgentOsWorkerLeaseV1Envelope({ ...valid, token: "secret" }),
    ...
    expect(() =>
      envelope("worker.availability", {
        ...
        identity: {
          ...
          audience: Array(1) as readonly [string],
        },
      }),
    ).toThrow(AgentOsWorkerLeaseV1ContractError);
    expect(() =>
  ...
  test("rejects payload and envelope digest tamper", () => {
    ...
    expect(() =>
      parseAgentOsWorkerLeaseV1Envelope({
        ...valid,
        payloadDigest: digest("tampered"),
      }),
    ).toThrow("DIGEST_MISMATCH");
    expect(() =>
      parseAgentOsWorkerLeaseV1Envelope({ ...valid, tenantId: "tenant.other" }),
    ).toThrow("DIGEST_MISMATCH");
  });
  ...
  test("rejects Grant audience, scope and validity expansion", () => {
    ...
    expect(() =>
      offer({ ...grant, audience: ["worker-1", "worker-2"] }),
    ).toThrow("GRANT_EXPANSION");
    expect(() =>
      offer({
        ...grant,
  ...
  test("rejects unsafe counters, invalid time windows and inconsistent terminal facts", () => {
    ...
    expect(() =>
      envelope("worker.availability", {
        ...availability,
        observedAt: "2026-08-07T00:01:00Z",
      }),
    ).toThrow(AgentOsWorkerLeaseV1ContractError);
    expect(() =>
      envelope("claim.ack", {
  ...
  test("rejects unsafe or authority-drifting resource health observations", () => {
    ...
    expect(() =>
      envelope("execution.resource-health", { ...resourceHealth, revision: 0 }),
    ).toThrow(AgentOsWorkerLeaseV1ContractError);
    expect(() =>
      envelope("execution.resource-health", {
        ...resourceHealth,
