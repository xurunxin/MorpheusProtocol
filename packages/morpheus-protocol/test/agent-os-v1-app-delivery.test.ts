...
describe("Agent OS v1 App delivery DTO authority", () => {
  test("strictly parses, copies and freezes an atomic App projection page", () => {
    ...
    expect(() =>
      parseAgentOsV1AppProjectionPage({ ...input, cacheKey: "local" }),
  ...
  test("owns strict versioned JSONL frame parsing and canonical serialization", () => {
    ...
    expect(serializeAgentOsV1TerminalFrame(frame)).toBe(
      `${JSON.stringify(frame)}\n`,
    );
    expect(() =>
      parseAgentOsV1TerminalFrame({ ...frame, exitCode: 0 }),
    ).toThrow(AgentOsV1ContractError);
    expect(() =>
      parseAgentOsV1TerminalFrame({ ...frame, token: "secret" }),
    ).toThrow(AgentOsV1ContractError);
  });
  ...
  test("fails closed when confirmation or step-up proof exceeds the named five-minute TTL", () => {
    expect(AGENT_OS_V1_DESTRUCTIVE_CONFIRMATION_MAX_TTL_MS).toBe(
      5 * 60 * 1_000,
    );
    expect(AGENT_OS_V1_DESTRUCTIVE_STEP_UP_PROOF_MAX_TTL_MS).toBe(
      5 * 60 * 1_000,
    );
    const confirmation = {
    ...
    expect(
      parseAgentOsV1DestructiveCommandConfirmation(confirmation).expiresAt,
    ).toBe(confirmation.expiresAt);
    expect(() =>
      parseAgentOsV1DestructiveCommandConfirmation({
        ...confirmation,
    ...
    expect(parseAgentOsV1DestructiveCommandStepUpProof(proof).expiresAt).toBe(
      proof.expiresAt,
    );
    expect(() =>
      parseAgentOsV1DestructiveCommandStepUpProof({
