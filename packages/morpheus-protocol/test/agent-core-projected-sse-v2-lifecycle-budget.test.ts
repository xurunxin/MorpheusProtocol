...
async function collect(
  ...
  for await (const event of stream)
    events.push({ cursor: event.cursor, event: event.event });
  return events;
}
...
describe("agent core projected SSE v2 lifecycle", () => {
    ...
    const result = clientForResponse(
      responseWithReader({
        async read() {
          reads += 1;
          if (reads === 1)
            return { done: false, value: encoder.encode(textDelta(1)) };
  ...
  it("cancels and releases when an in-flight read observes abort", async () => {
        ...
        read() {
          ...
          if (reads === 1)
            return Promise.resolve({
              ...
              value: encoder.encode(textDelta(1)),
            });
          notifyReadStarted?.();
    ...
    const stream = result.streamPrompt("agent", request, {
  ...
  it("cancels and releases an open stream after done without delivering a delayed duplicate", async () => {
    ...
    const delivered = await completesWithin(
      collect(result.streamPrompt("agent", request)),
      "done",
    );
...
describe("agent core projected SSE v2 budget boundaries", () => {
  it("accepts exactly the JSON depth limit and rejects one additional level", () => {
    ...
    for (
      let index = 0;
      index < agentCoreProjectedSseV2Limits.maxJsonDepth;
      index += 1
    ) {
      exact = [exact];
    }
  ...
  it("enforces array, object, and shared node limits at exact plus one", () => {
    ...
    const exactObject = Object.fromEntries(
      Array.from(
        { length: agentCoreProjectedSseV2Limits.maxObjectProperties },
    ...
    protocolCode(
      expectProtocolFailure(() =>
        parsePublicJsonValue({ ...exactObject, overflow: null }),
      ),
      "JSON_BUDGET",
    );

    const exactNodes = [
      ...Array.from({ length: 9_999 }, () => ({ value: null })),
      null,
    ];
    const tooManyNodes = Array.from({ length: 10_000 }, () => ({
      value: null,
    }));
  ...
  it("rejects root, object, and array accessors without invoking them", () => {
    ...
    inputs.push(root);
    const object = { nested: {} };
    Object.defineProperty(object.nested, "value", {
      enumerable: true,
      get: () => ++calls,
    });
    inputs.push(object);
    const array: unknown[] = [{}];
    Object.defineProperty(array[0] as object, "value", {
      enumerable: true,
      get: () => ++calls,
    });
    inputs.push(array);
  ...
  it("accepts offset leap seconds and arbitrary fractions, then rejects an invalid leap date", () => {
    for (const createdAt of [
      ...
      expect(
        parseAgentCoreProjectedSseEvent(
          eventInput("done", { sessionId: "session" }, { createdAt }),
        ).data.createdAt,
      ).toBe(createdAt);
    }
    protocolCode(
      expectProtocolFailure(() =>
        parseAgentCoreProjectedSseEvent(
          eventInput(
            "done",
            { sessionId: "session" },
            { createdAt: "2018-01-01T00:00:60Z" },
          ),
        ),
      ),
      "INVALID_VALUE",
  ...
  it("accepts an exact 64 KiB error body and rejects one additional byte", async () => {
    ...
    const exactBody = JSON.stringify({
      error: {
        ...
        message: "m",
        details: "x".repeat(
          agentCoreProjectedSseV2Limits.maxErrorBodyBytes -
            emptyEnvelope.length,
        ),
      },
    });
    expect(encoder.encode(exactBody)).toHaveLength(
      agentCoreProjectedSseV2Limits.maxErrorBodyBytes,
    );
    let exactReads = 0;
      ...
      responseWithReader(
        {
          read: async () => ({
            ...
            value: new Uint8Array(
              agentCoreProjectedSseV2Limits.maxErrorBodyBytes + 1,
            ),
          }),
          cancel: async () => {
            cancelled += 1;
