...
describe("agent-os-worker-lease/v1 reference layer", () => {
  ...
  test("strictly reparses an injected handler response", async () => {
    const request = availability();
    await expect(
      dispatchAgentOsWorkerLeaseV1Reference(request, () => request),
    ).resolves.toEqual(request);
    await expect(
      dispatchAgentOsWorkerLeaseV1Reference(request, () => ({
        ...request,
        token: "secret",
      })),
  ...
  test("fails closed on malformed, uncorrelated, or impossible checkpoints", async () => {
    ...
    for (const response of [
      ...
      await expect(client.readCheckpoint(request)).rejects.toBeInstanceOf(
        Error,
      );
    }
  });
  ...
  test("publishes the complete deterministic conformance matrix", async () => {
    ...
    expect(visited).toEqual(
      AGENT_OS_WORKER_LEASE_V1_CONFORMANCE_SCENARIOS.map(({ id }) => id),
    );
