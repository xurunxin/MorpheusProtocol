...
const PROJECTION = {
  generation: 7,
  projectionDigest:
    "sha256:1bd3650b9fc4017a3b28d10af1d5565a2be899ed3c5a502d8516c2f6a19c6bfa",
...
function expectContractError(
  ...
  parse: (
    input: unknown,
  ) => unknown = parseAgentOsV1SessionProjectionReadRequest,
): void {
  try {
    parse(value);
...
describe("agent-os session projection v1 request", () => {
  ...
  test("rejects unknown or missing fields and all non-session operations", () => {
    ...
    expectContractError(
      { ...REQUEST, operation: "session.write" },
      "INPUT_INVALID",
    );
    expectContractError(
  ...
  test("fails closed for cycles, depth, width, and byte budgets", () => {
    ...
    Object.defineProperty(tooLargeKey, "4294967295", {
      value: "bad",
      enumerable: true,
    });
  ...
  test("rejects a wrapper getter or unknown wrapper field without invoking it", () => {
    let invoked = false;
    const wrapper = { requestId: "request.demo", request: REQUEST } as Record<
      string,
      unknown
    >;
...
describe("agent-os session projection v1 binding", () => {
  test("uses an independently worked canonical digest for the projection and request", () => {
    ...
    expect(
      ...
      "sha256:3590685dfb813aec9a9b4f2a787148453697ef9874dec9f0b31f67f9bccf5e0d",
    );
  });
  ...
  test("binds request identity and classifies exact replay versus independent requests", () => {
    ...
    expect(
      assertAgentOsV1SessionProjectionRequestBindingCompatible(first, replay),
    ).toBe("replay");
    expect(
      assertAgentOsV1SessionProjectionRequestBindingCompatible(
        first,
  ...
  test("rejects same requestId drift as a stable idempotency conflict", () => {
    ...
    expect(() =>
      assertAgentOsV1SessionProjectionRequestBindingCompatible(first, drifted),
    ...
    expect(() =>
      assertAgentOsV1SessionProjectionRequestBindingCompatible(first, forged),
    ).toThrow("IDEMPOTENCY_CONFLICT");
  });
