...
export type AgentCoreProjectedSseEvent =
  ...
  | Frame<
      "text_start",
      { readonly contentIndex: number; readonly partial: AssistantPartial }
    >
  | Frame<
      "text_delta",
      {
...
export type AgentCoreProjectedSseEventName =
  AgentCoreProjectedSseEvent["event"];
...
export function parseAgentCoreProjectedPromptStreamRequest(
  ...
  if (query.trim().length === 0)
    fail("INVALID_REQUEST", "query must not be blank");
  if (scalarLength(requestId) < 1 || scalarLength(requestId) > 256) {
    fail(
      "INVALID_REQUEST",
      "requestId must contain 1..256 Unicode scalar values",
    );
  }
...
export function parseAgentCoreProjectedSseEvent(
  input: unknown,
): AgentCoreProjectedSseEvent {
  ...
  if (data.schemaVersion !== "agent-core.prompt-stream.v2")
    fail("INVALID_SCHEMA");
  ...
  if (event === "session" || event === "done") {
    ...
    if (sessionPayload.sessionId !== meta.sessionId) {
      fail(
        "SESSION_MISMATCH",
        `${event} payload sessionId does not match SSE data`,
      );
    }
  }
...
function parsePayload(event: string, payload: Record<string, unknown>): object {
  switch (event) {
    case "session":
    case "done":
      return strict(payload, ["sessionId"], {
...
function partial(input: unknown): AssistantPartial {
  ...
  return Object.freeze({
    role: "assistant",
    content: Object.freeze(items.map(content)),
  });
}
...
function toolCall(
  input: unknown,
): Extract<PublicContent, { type: "toolCall" }> {
...
function object(input: unknown, label: string): Record<string, unknown> {
  if (ArrayBuffer.isView(input)) {
    const length =
      "length" in input && typeof input.length === "number" ? input.length : 0;
...
function timestamp(value: unknown): string {
  ...
  if (timezone === undefined)
    fail("INVALID_VALUE", "createdAt timezone is required");
  if (
    month < 1 ||
    month > 12 ||
    day < 1 ||
  ...
  if (second === 60) {
    ...
    if (
      !publishedLeapSecondDates.has(utcDate) ||
      utc.hour !== 23 ||
      utc.minute !== 59
    )
  ...
  if (timezone !== "Z") {
    ...
    if (offsetHour > 23 || offsetMinute > 59)
      fail("INVALID_VALUE", "createdAt offset is invalid");
  }
  return input;
}
...
function boolean(value: unknown, label: string): boolean {
  if (typeof value !== "boolean")
    fail("INVALID_VALUE", `${label} must be a boolean`);
  return value;
}
...
function expect(value: unknown, expected: string, label: string): string {
  if (value !== expected)
    fail("INVALID_VALUE", `${label} must equal ${expected}`);
  return expected;
}
...
function assertStringBudget(value: string, label: string): void {
  if (
    ...
    agentCoreProjectedSseV2Limits.maxStringUtf8Bytes
  ) {
