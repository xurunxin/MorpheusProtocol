...
export function parseAgentOsBackupAdmissionApplicationV1(
  ...
  const applicationRef = ref(
    ...
    "backup admission applicationRef",
  );
  const applicationDigest = digest(
    value.applicationDigest,
    "backup admission applicationDigest",
  );
  if (applicationRef !== createAgentOsBackupAdmissionApplicationRefV1(unsigned))
    fail(
      "DRIFT_DETECTED",
      "backup admission applicationRef is self-inconsistent",
    );
  if (
    applicationDigest !==
    createAgentOsBackupAdmissionApplicationDigestV1(unsigned)
  )
    fail(
      "DRIFT_DETECTED",
      "backup admission applicationDigest is self-inconsistent",
    );
...
export function createAgentOsBackupAdmissionApplicationRefV1(
  input: unknown,
): string {
  const value = applicationUnsigned(
    record(input, "backup admission application unsigned"),
  );
  return `agent-os-backup-admission/v1/${value.commandId}/${value.policyDigest}/${value.checkpointDigest}`;
}

export function createAgentOsBackupAdmissionApplicationDigestV1(
  input: unknown,
): string {
...
export function serializeAgentOsBackupAdmissionApplicationV1(
  input: unknown,
): string {
...
function applicationUnsigned(
  ...
  return deepFreeze({
    ...
    checkpointDigest: digest(
      ...
      "backup admission checkpointDigest",
    ),
  });
}
