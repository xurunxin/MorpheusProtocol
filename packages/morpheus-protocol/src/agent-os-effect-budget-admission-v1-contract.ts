...
export function parseAgentOsEffectBudgetAdmissionApplicationV1(
  ...
  const applicationRef = ref(
    ...
    "effect budget admission applicationRef",
  );
  ...
  if (
    applicationRef !==
    createAgentOsEffectBudgetAdmissionApplicationRefV1(unsigned)
  )
    fail(
      "DRIFT_DETECTED",
      "effect budget admission applicationRef is self-inconsistent",
    );
  if (
    applicationDigest !==
    ...
    fail(
      "DRIFT_DETECTED",
      "effect budget admission applicationDigest is self-inconsistent",
    );
...
export function createAgentOsEffectBudgetAdmissionApplicationRefV1(
  input: unknown,
): string {
  const value = applicationUnsigned(
    record(input, "effect budget admission application unsigned"),
  );
...
export function createAgentOsEffectBudgetAdmissionApplicationDigestV1(
  input: unknown,
): string {
...
export function serializeAgentOsEffectBudgetAdmissionApplicationV1(
  input: unknown,
): string {
...
function applicationUnsigned(
  ...
  return deepFreeze({
    ...
    reservationId: identifier(
      value.reservationId,
      "effect budget admission reservationId",
    ),
    requestDigest: digest(
      value.requestDigest,
      "effect budget admission requestDigest",
    ),
    ...
    kernelFenceDigest: digest(
      ...
      "effect budget admission kernelFenceDigest",
    ),
  });
}
...
function ref(value: unknown, label: string): string {
  if (typeof value !== "string" || !APPLICATION_REF_PATTERN.test(value))
    fail(
      "INVALID_VALUE",
      `${label} must be a canonical path-free application ref`,
    );
  return value;
}
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value))
      fail("INVALID_VALUE", "canonical source is invalid");
    return JSON.stringify(value);
  }
