...
export function parseAgentOsRunTreeBudgetCeilingV1(
  ...
  const ceilingDigest = selfDigest(
    value.ceilingDigest,
    unsigned,
    "budget ceiling ceilingDigest",
  );
...
export function parseAgentOsBudgetReservationRequestV1(
  ...
  const unsigned = reservationRequestUnsigned(
    omitField(value, "requestDigest"),
  );
...
export function parseAgentOsBudgetReservationReceiptV1(
  ...
  const unsigned = reservationReceiptUnsigned(
    omitField(value, "receiptDigest"),
  );
...
export function parseAgentOsBudgetCurrentStateV1(
  ...
  exact(
    value,
    [...CURRENT_STATE_UNSIGNED_KEYS, "stateDigest"],
    "budget current state",
  );
  const unsigned = currentStateUnsigned(omitField(value, "stateDigest"));
  const stateDigest = selfDigest(
    value.stateDigest,
    unsigned,
    "budget current state stateDigest",
  );
...
export function parseAgentOsBudgetSettlementReceiptV1(
  ...
  exact(
    value,
    [...SETTLEMENT_UNSIGNED_KEYS, "receiptDigest"],
...
export function parseAgentOsResourceAttributionReceiptV1(
  ...
  exact(
    value,
    [...ATTRIBUTION_UNSIGNED_KEYS, "receiptDigest"],
...
export function parseAgentOsBudgetCeilingSuccessorV1(
  ...
  exact(
    value,
    [...SUCCESSOR_UNSIGNED_KEYS, "successorDigest"],
    "budget ceiling successor",
  );
...
export function serializeAgentOsBudgetReservationRequestV1(
  input: unknown,
): string {
  return canonicalJson(parseAgentOsBudgetReservationRequestV1(input));
}

export function serializeAgentOsBudgetReservationReceiptV1(
  input: unknown,
): string {
...
export function serializeAgentOsBudgetSettlementReceiptV1(
  input: unknown,
): string {
  return canonicalJson(parseAgentOsBudgetSettlementReceiptV1(input));
}

export function serializeAgentOsResourceAttributionReceiptV1(
  input: unknown,
): string {
  return canonicalJson(parseAgentOsResourceAttributionReceiptV1(input));
}

export function serializeAgentOsBudgetCeilingSuccessorV1(
  input: unknown,
): string {
  return canonicalJson(parseAgentOsBudgetCeilingSuccessorV1(input));
}

export function createAgentOsRunTreeBudgetCeilingDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(ceilingUnsigned(record(input, "budget ceiling unsigned"))),
  );
}

export function createAgentOsBudgetReservationRequestDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      reservationRequestUnsigned(
        record(input, "budget reservation request unsigned"),
      ),
    ),
  );
}

export function createAgentOsBudgetReservationReceiptDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      reservationReceiptUnsigned(
        record(input, "budget reservation receipt unsigned"),
      ),
    ),
  );
}

export function createAgentOsBudgetCurrentStateDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      currentStateUnsigned(record(input, "budget current state unsigned")),
    ),
  );
}

export function createAgentOsBudgetSettlementMutationDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      settlementMutationUnsigned(
        record(input, "budget settlement mutation unsigned"),
      ),
    ),
  );
}

export function createAgentOsBudgetSettlementReceiptDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      settlementUnsigned(record(input, "budget settlement receipt unsigned")),
    ),
  );
}

export function createAgentOsResourceAttributionReceiptDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      attributionUnsigned(
        record(input, "resource attribution receipt unsigned"),
      ),
    ),
  );
}

export function createAgentOsBudgetCeilingSuccessorDigestV1(
  input: unknown,
): string {
  return contentDigest(
    canonicalJson(
      successorUnsigned(record(input, "budget ceiling successor unsigned")),
    ),
  );
}
...
export function createAgentOsBudgetChargeKeyV1(
  input: AgentOsBudgetChargeIdentityInputV1,
): string {
  return contentDigest(
    canonicalJson({
      schemaVersion: SCHEMA_VERSION,
      kind: "charge",
      attributionKey: digest(
        input.attributionKey,
        "budget charge attributionKey",
      ),
      reservationId: identifier(
        input.reservationId,
        "budget charge reservationId",
      ),
...
export function assertAgentOsBudgetReservationRelationshipV1(
  ...
  same(
    request.ceilingDigest,
    ceiling.ceilingDigest,
    "reservation ceilingDigest drift",
  );
  same(
    request.expectedCeilingRevision,
    ceiling.revision,
    "reservation ceiling revision drift",
  );
  same(
    request.balanceStateDigest,
    currentState.stateDigest,
    "reservation balance state drift",
  );
  same(
  ...
  same(
    ...
    "reservation request digest drift",
  );
  same(receipt.ceilingId, ceiling.ceilingId, "receipt ceilingId drift");
  same(
    receipt.ceilingDigest,
    ceiling.ceilingDigest,
    "receipt ceilingDigest drift",
  );
  same(
    receipt.ceilingRevision,
    ceiling.revision,
    "receipt ceiling revision drift",
  );
  same(
    receipt.balanceStateDigest,
    currentState.stateDigest,
    "receipt balance state drift",
  );
  same(
  ...
  same(
    receipt.attributionKey,
    request.attributionKey,
    "reservation attribution drift",
  );
  same(receipt.chargeKey, request.chargeKey, "reservation charge drift");
  assertVectorEqual(
    receipt.availableBefore,
    available,
    "reservation availableBefore drift",
  );
  same(
    currentState.ceilingId,
    ceiling.ceilingId,
    "current state ceilingId drift",
  );
  same(
    currentState.ceilingDigest,
    ceiling.ceilingDigest,
    "current state ceilingDigest drift",
  );
  same(
    currentState.ceilingRevision,
    ceiling.revision,
    "current state ceiling revision drift",
  );
  ...
  if (parent === null) {
    ...
    if (
      ...
      fail(
        "DRIFT_DETECTED",
        "root reservation requires canonical ceiling balance state",
      );
    assertVectorEqual(
      currentState.reserved,
      ceiling.limit,
      "root balance capacity drift",
    );
    assertVectorWithin(
      request.requested,
      ceiling.limit,
      "root reservation exceeds ceiling",
    );
  } else {
    if (parent.disposition !== "reserved")
      fail(
        "INVALID_VALUE",
        "child reservation requires an active reserved parent",
      );
    ...
    same(
      parent.ceilingDigest,
      ceiling.ceilingDigest,
      "parent ceilingDigest drift",
    );
    same(
      parent.ceilingRevision,
      ceiling.revision,
      "parent ceiling revision drift",
    );
    same(
      request.parentReservationId,
      parent.reservationId,
      "parent reservationId drift",
    );
    same(
      request.parentReservationDigest,
      parent.receiptDigest,
      "parent reservation digest drift",
    );
    same(
      request.parentAttributionKey,
      parent.attributionKey,
      "parent attribution drift",
    );
    same(
      receipt.parentReservationId,
      parent.reservationId,
      "receipt parent drift",
    );
    if (currentState.ownerDisposition !== "reserved")
      fail(
        "INVALID_VALUE",
        "child reservation requires active parent current state",
      );
    same(
      currentState.ownerReservationId,
      parent.reservationId,
      "current parent identity drift",
    );
    same(
  ...
  same(
    request.attributionKey,
    expectedAttribution,
    "reservation attribution key drift",
  );
  same(
    request.chargeKey,
  ...
  if (receipt.disposition === "reserved") {
    if (!fits)
      fail(
        "GRANT_EXPANSION",
        "reserved receipt oversells current available budget",
      );
    ...
    assertVectorEqual(
      receipt.reserved,
      ZERO_VECTOR,
      "denied receipt must reserve zero",
    );
    assertVectorEqual(
      receipt.availableAfter,
      available,
      "denied receipt changed available budget",
    );
  }
}
...
export function assertAgentOsBudgetSettlementRelationshipV1(
  ...
  const sourceCommit =
    input.sourceCommit === null
      ? null
      : parseAgentOsBudgetSettlementReceiptV1(input.sourceCommit);
  same(
    receipt.reservationId,
    reservation.reservationId,
    "settlement reservationId drift",
  );
  same(
  ...
  same(
    currentState.ceilingId,
    reservation.ceilingId,
    "settlement current ceilingId drift",
  );
  same(
    currentState.ceilingDigest,
  ...
  if (currentState.ownerDisposition !== "reserved")
    fail(
      "INVALID_VALUE",
      "settlement requires an active reservation current state",
    );
  assertVectorEqual(
    currentState.reserved,
    reservation.reserved,
    "current reserved amount drift",
  );
  same(
    receipt.previousStateDigest,
    currentState.stateDigest,
    "settlement previous state drift",
  );
  same(
  ...
  assertVectorEqual(
    ...
    expectedCommitted,
    "settlement committed total drift",
  );
  assertVectorEqual(
    receipt.releasedTotal,
    expectedReleased,
    "settlement released total drift",
  );
  assertVectorEqual(
    receipt.refundedTotal,
    expectedRefunded,
    "settlement refunded total drift",
  );
  assertVectorWithin(
  ...
  if (receipt.operation === "commit") {
    if (
      receipt.usageEvidenceDigest === null ||
      receipt.correctionEvidenceDigest !== null
    )
      fail(
        "DRIFT_DETECTED",
        "commit requires usage evidence and forbids correction evidence",
      );
    if (
      sourceCommit !== null ||
    ...
    if (
      ...
      fail(
        "DRIFT_DETECTED",
        "release must not fabricate usage or correction evidence",
      );
    if (currentState.commitStates.length !== 0)
      fail(
        "DRIFT_DETECTED",
        "release is forbidden after any committed side-effect boundary",
      );
    if (
      sourceCommit !== null ||
      receipt.sourceCommitReceiptDigest !== null ||
    ...
    if (
      ...
      fail(
        "DRIFT_DETECTED",
        "refund requires usage and authoritative correction evidence",
      );
    ...
    same(
      sourceCommit.reservationId,
      reservation.reservationId,
      "refund source reservation drift",
    );
    same(
    ...
    if (sourceState === undefined)
      fail(
        "DRIFT_DETECTED",
        "refund source commit is absent from current state",
      );
    same(
    ...
    assertVectorEqual(
      ...
      sourceCommit.amount,
      "refund source amount drift",
    );
    const expectedSourceRefunded = addVector(
...
export function assertAgentOsResourceAttributionRelationshipV1(
  ...
  const attribution = parseAgentOsResourceAttributionReceiptV1(
    input.attribution,
  );
  same(
    request.requestDigest,
    reservation.requestDigest,
    "attribution request digest drift",
  );
  same(
    request.reservationId,
    reservation.reservationId,
    "attribution request identity drift",
  );
  same(ceiling.ceilingId, reservation.ceilingId, "attribution ceilingId drift");
  same(
    ceiling.ceilingDigest,
    reservation.ceilingDigest,
    "attribution ceilingDigest drift",
  );
  same(
    ceiling.revision,
    reservation.ceilingRevision,
    "attribution ceiling revision drift",
  );
  same(
    currentState.ceilingId,
    ceiling.ceilingId,
    "attribution current ceilingId drift",
  );
  same(
    currentState.ceilingDigest,
    ceiling.ceilingDigest,
  ...
  if (currentState.ownerDisposition !== "reserved")
    fail(
      "INVALID_VALUE",
      "attribution requires an active reservation current state",
    );
  if (request.subject.kind !== "effect")
    fail(
      "INVALID_VALUE",
      "resource attribution requires an effect reservation",
    );
  same(
    attribution.reservationId,
    reservation.reservationId,
    "attribution reservation drift",
  );
  same(
  ...
  same(
    ...
    reservation.attributionKey,
    "attribution key drift",
  );
  same(
    attribution.attemptId,
    request.subject.attemptId,
    "attribution Attempt identity drift",
  );
  same(
    attribution.effectId,
    request.subject.effectId,
    "attribution Effect identity drift",
  );
  same(
    effectDispatchReceipt.runId,
    request.subject.runId,
    "dispatch Run identity drift",
  );
  same(
    effectDispatchReceipt.attemptId,
    attribution.attemptId,
    "dispatch Attempt identity drift",
  );
  same(
    effectDispatchReceipt.effectId,
    attribution.effectId,
    "dispatch Effect identity drift",
  );
  same(
  ...
  same(
    attribution.chargeKey,
    expectedChargeKey,
    "attribution charge identity drift",
  );
  same(
    reservation.chargeKey,
    expectedChargeKey,
    "reservation charge identity drift",
  );
  assertVectorWithin(
  ...
  for (const dimension of DIMENSIONS) {
    ...
    const charge = vectorForDimension(
      attribution.conservativeCharge,
      dimension,
    );
    ...
    if (observation.kind === "known" && charge < observation.value)
      fail(
        "GRANT_EXPANSION",
        `${dimension} charge cannot be below known usage`,
      );
    if (
...
export function assertAgentOsBudgetCeilingSuccessorRelationshipV1(
  ...
  same(
    successor.predecessorCeilingId,
    predecessor.ceilingId,
    "successor predecessorId drift",
  );
  same(
  ...
  same(
    successor.predecessorRevision,
    predecessor.revision,
    "successor predecessor revision drift",
  );
  same(
    successor.successor.ceilingId,
    predecessor.ceilingId,
    "successor ceiling identity drift",
  );
  same(
    successor.successor.tenantId,
    predecessor.tenantId,
    "successor tenant drift",
  );
  same(
    successor.successor.workloadId,
    predecessor.workloadId,
    "successor workload drift",
  );
  same(
    successor.successor.rootRunId,
    predecessor.rootRunId,
    "successor root Run drift",
  );
  same(
    successor.successor.policyDigest,
    predecessor.policyDigest,
    "successor policy drift",
  );
  same(
...
function ceilingUnsigned(
  ...
  return deepFreeze({
    ...
    hardDimensions: budgetDimensions(
      value.hardDimensions,
      "budget ceiling hardDimensions",
    ),
...
function reservationRequestUnsigned(
  ...
  exact(
    value,
    RESERVATION_REQUEST_UNSIGNED_KEYS,
    "budget reservation request unsigned",
  );
  schema(value.schemaVersion, "budget reservation request");
  if (value.operation !== "reserve")
    fail(
      "INVALID_VALUE",
      "budget reservation request operation must equal reserve",
    );
  const subject = budgetSubject(
    value.subject,
    "budget reservation request subject",
  );
  ...
  if (
    ...
    fail(
      "INVALID_SHAPE",
      "budget reservation parent lineage must be all-null or complete",
    );
  ...
  if ((subject.kind === "effect") !== (effectPermitDigest !== null))
    fail(
      "DRIFT_DETECTED",
      "only effect reservations require effectPermitDigest",
    );
  return deepFreeze({
    schemaVersion: SCHEMA_VERSION,
    operation: "reserve",
    commandId: identifier(
      value.commandId,
      "budget reservation request commandId",
    ),
    reservationId: identifier(
      value.reservationId,
      "budget reservation request reservationId",
    ),
    ceilingId: identifier(
      value.ceilingId,
      "budget reservation request ceilingId",
    ),
    ceilingDigest: digest(
      value.ceilingDigest,
      "budget reservation request ceilingDigest",
    ),
    ...
    requested: budgetVector(
      value.requested,
      "budget reservation request requested",
    ),
    ...
    attributionKey: digest(
      ...
      "budget reservation request attributionKey",
    ),
    chargeKey: digest(value.chargeKey, "budget reservation request chargeKey"),
    requestedAt: instant(
      value.requestedAt,
      "budget reservation request requestedAt",
    ),
  });
}
...
function reservationReceiptUnsigned(
  ...
  exact(
    value,
    RESERVATION_RECEIPT_UNSIGNED_KEYS,
    "budget reservation receipt unsigned",
  );
  schema(value.schemaVersion, "budget reservation receipt");
  if (value.operation !== "reserve")
    fail(
      "INVALID_VALUE",
      "budget reservation receipt operation must equal reserve",
    );
  const disposition =
    value.disposition === "reserved" || value.disposition === "denied"
      ...
      : fail(
          "INVALID_VALUE",
          "budget reservation receipt disposition is invalid",
        );
  return deepFreeze({
    schemaVersion: SCHEMA_VERSION,
    operation: "reserve",
    receiptId: identifier(
      value.receiptId,
      "budget reservation receipt receiptId",
    ),
    commandId: identifier(
      value.commandId,
      "budget reservation receipt commandId",
    ),
    reservationId: identifier(
      value.reservationId,
      "budget reservation receipt reservationId",
    ),
    requestDigest: digest(
      value.requestDigest,
      "budget reservation receipt requestDigest",
    ),
    disposition,
    denialReason: denialReason(value.denialReason),
    ceilingId: identifier(
      value.ceilingId,
      "budget reservation receipt ceilingId",
    ),
    ceilingDigest: digest(
      value.ceilingDigest,
      "budget reservation receipt ceilingDigest",
    ),
    ...
    reserved: budgetVector(
      value.reserved,
      "budget reservation receipt reserved",
    ),
    availableBefore: budgetVector(
      value.availableBefore,
      "budget reservation receipt availableBefore",
    ),
    availableAfter: budgetVector(
      value.availableAfter,
      "budget reservation receipt availableAfter",
    ),
    attributionKey: digest(
      value.attributionKey,
      "budget reservation receipt attributionKey",
    ),
    chargeKey: digest(value.chargeKey, "budget reservation receipt chargeKey"),
    committedAt: instant(
      value.committedAt,
      "budget reservation receipt committedAt",
    ),
  });
}

function currentStateUnsigned(
  value: Record<string, unknown>,
): AgentOsBudgetCurrentStateUnsignedV1 {
  ...
  if (
    ...
    fail(
      "INVALID_SHAPE",
      "budget current state owner lineage must be all-null or complete",
    );
  if ((ownerDisposition === "ceiling") !== (ownerReservationId === null))
    fail(
      "INVALID_SHAPE",
      "only ceiling state may omit owner reservation lineage",
    );
  ...
  if (ownerDisposition === "ceiling" && reservationRevision !== 0)
    fail(
      "DRIFT_DETECTED",
      "ceiling current state reservationRevision must equal zero",
    );
  if (ownerDisposition !== "ceiling" && reservationRevision === 0)
    fail(
      "DRIFT_DETECTED",
      "reservation current state requires a positive revision",
    );

  const reserved = budgetVector(
    value.reserved,
    "budget current state reserved",
  );
  const available = budgetVector(
    value.available,
    "budget current state available",
  );
  const committedTotal = budgetVector(
    value.committedTotal,
    "budget current state committedTotal",
  );
  const releasedTotal = budgetVector(
    value.releasedTotal,
    "budget current state releasedTotal",
  );
  const refundedTotal = budgetVector(
    value.refundedTotal,
    "budget current state refundedTotal",
  );
  ...
  assertVectorWithin(
    available,
    reserved,
    "current available exceeds reserved capacity",
  );
  assertVectorWithin(
  ...
  assertVectorWithin(
    refundedTotal,
    committedTotal,
    "current refund exceeds committed charge",
  );
  ...
  assertVectorEqual(
    commitTotals,
    committedTotal,
    "commit state aggregate drift",
  );
  assertVectorEqual(
    refundTotals,
    refundedTotal,
    "refund state aggregate drift",
  );
  if (ownerDisposition === "ceiling") {
    if (commitStates.length !== 0)
      fail(
        "DRIFT_DETECTED",
        "ceiling balance state cannot contain settlement commits",
      );
    assertVectorEqual(
      committedTotal,
      ZERO_VECTOR,
      "ceiling committed total must be zero",
    );
    assertVectorEqual(
      releasedTotal,
      ZERO_VECTOR,
      "ceiling released total must be zero",
    );
    assertVectorEqual(
      refundedTotal,
      ZERO_VECTOR,
      "ceiling refunded total must be zero",
    );
  }
  ...
  return deepFreeze({
    ...
    ceilingDigest: digest(
      value.ceilingDigest,
      "budget current state ceilingDigest",
    ),
    ceilingRevision: nonNegativeInteger(
      value.ceilingRevision,
...
function commitStateValues(
  value: unknown,
): readonly Readonly<AgentOsBudgetCommitStateV1>[] {
  const values = arrayValues(value, "budget current state commitStates").map(
    (item, index) => {
      const state = record(item, `budget current state commitStates[${index}]`);
      exact(
        state,
        COMMIT_STATE_KEYS,
        `budget current state commitStates[${index}]`,
      );
      ...
      assertVectorWithin(
        refunded,
        committed,
        "per-commit refund exceeds committed charge",
      );
      return deepFreeze({
  ...
  for (let index = 1; index < values.length; index += 1) {
    if (
      values[index - 1]!.commitReceiptDigest >=
      values[index]!.commitReceiptDigest
    )
      fail(
        "INVALID_VALUE",
        "commitStates must be unique and sorted by commitReceiptDigest",
      );
  }
...
function settlementMutationUnsigned(
  ...
  return deepFreeze({
    ...
    commandId: identifier(
      value.commandId,
      "budget settlement mutation commandId",
    ),
    reservationId: identifier(
      value.reservationId,
      "budget settlement mutation reservationId",
    ),
    ...
    occurredAt: instant(
      value.occurredAt,
      "budget settlement mutation occurredAt",
    ),
  });
}
...
function settlementUnsigned(
  ...
  return deepFreeze({
    ...
    receiptId: identifier(
      value.receiptId,
      "budget settlement receipt receiptId",
    ),
    commandId: identifier(
      value.commandId,
      "budget settlement receipt commandId",
    ),
    reservationId: identifier(
      value.reservationId,
      "budget settlement receipt reservationId",
    ),
    reservationReceiptDigest: digest(
      value.reservationReceiptDigest,
    ...
    mutationDigest: digest(
      ...
      "budget settlement receipt mutationDigest",
    ),
    expectedReservationRevision: positiveInteger(
      value.expectedReservationRevision,
    ...
    committedTotal: budgetVector(
      value.committedTotal,
      "budget settlement receipt committedTotal",
    ),
    releasedTotal: budgetVector(
      value.releasedTotal,
      "budget settlement receipt releasedTotal",
    ),
    refundedTotal: budgetVector(
      value.refundedTotal,
      "budget settlement receipt refundedTotal",
    ),
    sourceCommitReceiptDigest: nullableDigest(
      value.sourceCommitReceiptDigest,
    ...
    occurredAt: instant(
      value.occurredAt,
      "budget settlement receipt occurredAt",
    ),
  });
}
...
function attributionUnsigned(
  ...
  exact(
    value,
    ATTRIBUTION_UNSIGNED_KEYS,
    "resource attribution receipt unsigned",
  );
  ...
  return deepFreeze({
    ...
    receiptId: identifier(
      value.receiptId,
      "resource attribution receipt receiptId",
    ),
    reservationId: identifier(
      value.reservationId,
      "resource attribution receipt reservationId",
    ),
    reservationReceiptDigest: digest(
      value.reservationReceiptDigest,
    ...
    attributionKey: digest(
      value.attributionKey,
      "resource attribution receipt attributionKey",
    ),
    chargeKey: digest(
      value.chargeKey,
      "resource attribution receipt chargeKey",
    ),
    attemptId: identifier(
      value.attemptId,
      "resource attribution receipt attemptId",
    ),
    effectId: identifier(
      value.effectId,
      "resource attribution receipt effectId",
    ),
    ...
    observed: observation(
      value.observed,
      "resource attribution receipt observed",
    ),
    ...
    observedAt: instant(
      value.observedAt,
      "resource attribution receipt observedAt",
    ),
  });
}
...
function successorUnsigned(
  ...
  return deepFreeze({
    ...
    installedAt: instant(
      value.installedAt,
      "budget ceiling successor installedAt",
    ),
  });
}
...
function budgetSubject(
  value: unknown,
  label: string,
): Readonly<AgentOsBudgetSubjectV1> {
  ...
  const subject = deepFreeze({
    ...
    storeGeneration: nonNegativeInteger(
      input.storeGeneration,
  ...
  if (
    ...
    (subject.turnId === null ||
      ...
      subject.effectId === null)
  )
    fail(
      "DRIFT_DETECTED",
      `${label} effect requires turnId, attemptId and effectId`,
    );
  return subject;
}
...
function budgetVector(
  value: unknown,
  label: string,
): Readonly<AgentOsBudgetVectorV1> {
  const input = record(value, label);
  exact(
    input,
    ["inputTokens", "outputTokens", "toolCalls", "costUsdMicros"],
    label,
  );
  return deepFreeze({
...
function observation(
  value: unknown,
  label: string,
): Readonly<AgentOsBudgetObservationV1> {
  const input = record(value, label);
  exact(
    input,
    ["inputTokens", "outputTokens", "toolCalls", "costUsdMicros"],
  ...
  return deepFreeze({
    ...
    costUsdMicros: observedQuantity(
      input.costUsdMicros,
      `${label}.costUsdMicros`,
    ),
  });
...
function observedQuantity(
  ...
  if (input.kind === "known") {
    ...
    return deepFreeze({
      kind: "known",
      value: nonNegativeInteger(input.value, `${label}.value`),
    });
  }
  if (input.kind === "unknown") {
    ...
    return deepFreeze({
      kind: "unknown",
      reason: unknownReason(input.reason, `${label}.reason`),
    });
  }
  fail("INVALID_VALUE", `${label}.kind is invalid`);
}
...
function budgetDimensions(
  value: unknown,
  label: string,
): readonly AgentOsBudgetDimensionV1[] {
  ...
  const order = new Map(
    DIMENSIONS.map((dimension, index) => [dimension, index]),
  );
  return deepFreeze(
    [...items].sort((left, right) => order.get(left)! - order.get(right)!),
  );
}
...
function denialReason(
  value: unknown,
): AgentOsBudgetReservationDenialReasonV1 | null {
...
function currentStateOwnerDisposition(
  value: unknown,
): AgentOsBudgetCurrentStateOwnerDispositionV1 {
  if (value === "ceiling" || value === "reserved" || value === "closed")
    return value;
  return fail(
    "INVALID_VALUE",
    "budget current state ownerDisposition is invalid",
  );
}
...
function subtractVector(
  ...
  if (!vectorWithin(right, left))
    fail("GRANT_EXPANSION", "budget vector subtraction underflow");
  return deepFreeze({
...
function checkedAdd(left: number, right: number, label: string): number {
  ...
  if (!Number.isSafeInteger(result))
    fail("INVALID_VALUE", `${label} sum exceeds safe integer`);
  return result;
}

function vectorWithin(
  actual: AgentOsBudgetVectorV1,
  ceiling: AgentOsBudgetVectorV1,
): boolean {
  return DIMENSIONS.every(
    (dimension) =>
      vectorForDimension(actual, dimension) <=
      vectorForDimension(ceiling, dimension),
  );
}
...
function schema(value: unknown, label: string): void {
  if (value !== SCHEMA_VERSION)
    fail(
      "UNSUPPORTED_VERSION",
      `${label} schemaVersion must equal ${SCHEMA_VERSION}`,
    );
}
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value))
      fail(
        "INVALID_VALUE",
        "canonical budget source contains a non-safe integer",
      );
    return JSON.stringify(value);
  }
  ...
  fail(
    "INVALID_VALUE",
    "canonical budget source contains an unsupported value",
  );
}
