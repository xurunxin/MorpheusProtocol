...
export function negotiateAgentOsV1Handshake(
  ...
  const commonVersions = client.protocol.versions
    .filter(
      (version) =>
        providerVersions.has(version) && registered.includes(version),
    )
  ...
  if (
    [
      ...client.protocol.requiredFeatures,
      ...provider.protocol.requiredFeatures,
    ].some((feature) => !selectedSet.has(feature))
  )
 ...
 */
export const AGENT_OS_V1_IMMUTABLE_PROMPT_REFERENCE_ARTIFACT =
  createImmutablePromptArtifact();
...
export function bindAgentOsV1ImmutablePromptReferenceEnvelope(
  ...
  const envelope =
    AGENT_OS_V1_IMMUTABLE_PROMPT_REFERENCE_ARTIFACT.envelopes[kind];
  return parseAgentOsV1AuthorityRequestEnvelope({
    ...envelope,
...
function createImmutablePromptArtifact() {
  ...
  const negotiation = negotiateAgentOsV1Handshake(
    clientOffer,
    providerOffer,
    nowEpochMs,
  );
...
export interface AgentOsV1CanonicalPromptReferenceClient {
  ...
  readonly request: (
    ...
    Readonly<AgentOsV1ReferenceResponse<AgentOsV1CanonicalPromptResponse>>
  >;
  readonly start: (
    envelope: unknown,
    snapshot: unknown,
...
export interface AgentOsV1CanonicalPromptReferenceHandler {
  ...
  readonly handle: (
    request: Readonly<
      AgentOsV1ReferenceRequest<AgentOsV1CanonicalPromptRequest>
    >,
  ) =>
    | Promise<AgentOsV1CanonicalPromptResponse>
    | AgentOsV1CanonicalPromptResponse;
}
...
> {
  ...
  if (
    request.protocolId !== "execution.v1" ||
    request.operation !== request.payload.operation
  )
    throw new AgentOsV1ReferenceError(
      "PIN_DRIFT",
...
function assertCanonicalPromptResponseCorrelation(
  ...
  if (
    (firstEvent !== undefined &&
      firstEvent.sequence !== request.cursor.sequence + 1) ||
    (firstEvent === undefined &&
      response.cursor.sequence !== request.cursor.sequence)
  )
    throw new AgentOsV1ReferenceError(
      "PIN_DRIFT",
...
export async function dispatchAgentOsV1Reference<TRequest, TResponse>(
  ...
  if (handler === undefined)
    throw new AgentOsV1ReferenceError(
      "UPDATE_REQUIRED",
      "pinned handler resolution failed",
    );
...
export type AgentOsV1HandlerResolution =
  | Readonly<{
      status: "accepted";
      handler: Readonly<AgentOsV1HandlerCatalogEntry>;
    }>
...
export function resolveAgentOsV1PinnedHandler(
  ...
  const handler = catalog.handlers.find(
    (entry) =>
      entry.protocolId === activePin.protocolId &&
      entry.handlerVersion === activePin.handlerVersion,
  );
...
export type AgentOsV1HandlerTransitionResult =
  | Readonly<{
      status: "accepted";
      snapshot: Readonly<AgentOsV1HandlerCatalogSnapshot>;
    }>
  | Readonly<{
      status: "UPDATE_REQUIRED";
      reason: "HANDLER_MISSING" | "HANDLER_PINNED";
    }>;
...
export function planAgentOsV1HandlerTransition(
  ...
  const index = catalog.handlers.findIndex(
    (handler) =>
      handler.protocolId === command.protocolId &&
      handler.handlerVersion === command.handlerVersion,
  );
  if (index < 0) return updateResult("UPDATE_REQUIRED", "HANDLER_MISSING");
  const pinned = activePins.some(
    (pin) =>
      pin.protocolId === command.protocolId &&
      pin.handlerVersion === command.handlerVersion,
  );
  ...
  if (catalog.revision === Number.MAX_SAFE_INTEGER)
    throw new AgentOsV1ReferenceError(
      "INVALID_INPUT",
      "handler catalog revision is exhausted",
    );
...
export type AgentOsV1PersonalTransitionResult =
  ...
  | Readonly<{
      ...
      reason:
        | "INVALID_TRANSITION"
        | "REMOTE_RENEWAL_DENIED"
        | "REVOKED_AUTO_RECOVERY_DENIED";
    }>;
...
export function planAgentOsV1PersonalTransition(
  ...
  const allowedTransitions: readonly PersonalHostState[] =
    PERSONAL_TRANSITIONS[input.from];
  if (!allowedTransitions.includes(input.to))
    return updateResult("rejected", "INVALID_TRANSITION");
...
function parseOwnerHandlers<TRequest, TResponse>(
  ...
  const identities = handlers.map(
    (handler) =>
      `${handler.protocolId}:${handler.handlerVersion}:${handler.operation}`,
  );
  if (new Set(identities).size !== identities.length)
    throw new AgentOsV1ReferenceError(
      "INVALID_INPUT",
      "owner handlers contain duplicate keys",
    );
...
function parseCanonicalPromptHandlers(
  ...
  const handlers = strictArrayValues(input, "canonical prompt handlers").map(
    (entry) => {
      ...
      return Object.freeze({
        ...
        handle:
          value.handle as AgentOsV1CanonicalPromptReferenceHandler["handle"],
      });
    },
  );
  const identities = handlers.map(
...
function validatePeers(
  ...
  if (
    Math.abs(Date.parse(client.issuedAt) - nowEpochMs) >
      client.maxClockSkewMs ||
    Math.abs(Date.parse(provider.issuedAt) - nowEpochMs) >
      provider.maxClockSkewMs
  )
    return "CLOCK_SKEW";
  return null;
}
...
function sameSnapshot(
  ...
  return (
    ...
    snapshot.selectedFeatures.every(
      (feature, index) => feature === pin.selectedFeatures[index],
    )
  );
}
...
function assertLiveDeadline(deadline: string, nowEpochMs: number): void {
  ...
  if (Date.parse(deadline) <= nowEpochMs)
    throw new AgentOsV1ReferenceError(
      "DEADLINE_EXPIRED",
      "authority request deadline has expired",
    );
}

function assertProtocolFamily(
  value: unknown,
): asserts value is AgentOsV1ProtocolFamily {
  if (
    typeof value !== "string" ||
    !Object.hasOwn(AGENT_OS_V1_PROTOCOL_REGISTRY, value)
  )
    throw new AgentOsV1ReferenceError(
      "INVALID_INPUT",
      "protocolId is not registered",
    );
}
...
function updateResult<
  TStatus extends "UPDATE_REQUIRED" | "rejected",
