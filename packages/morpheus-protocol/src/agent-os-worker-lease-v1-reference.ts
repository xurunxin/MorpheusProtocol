...
export function createAgentOsWorkerLeaseV1ReferenceClient(
  ...
  return Object.freeze({
    ...
    async readCheckpoint(
      requestInput: AgentOsWorkerLeaseV1CheckpointRequestInput,
    ) {
...
export const AGENT_OS_WORKER_LEASE_V1_CONFORMANCE_SCENARIOS = deepFreeze([
  {
    ...
    expected: "reject",
    rejectionCode: "artifact_corrupt",
  },
...
export interface AgentOsWorkerLeaseV1ConformanceExecutor {
  readonly execute: (
    ...
    | AgentOsWorkerLeaseV1ConformanceResult
    | Promise<AgentOsWorkerLeaseV1ConformanceResult>;
}
...
function parseCheckpointRequest(
  input: unknown,
): Readonly<AgentOsWorkerLeaseV1CheckpointRequest> {
...
function parseCheckpoint(
  ...
  if (
    !Number.isSafeInteger(value.leaderTerm) ||
    (value.leaderTerm as number) <= 0
  )
...
function isScenarioId(
  value: unknown,
): value is AgentOsWorkerLeaseV1ConformanceScenarioId {
  return AGENT_OS_WORKER_LEASE_V1_CONFORMANCE_SCENARIOS.some(
