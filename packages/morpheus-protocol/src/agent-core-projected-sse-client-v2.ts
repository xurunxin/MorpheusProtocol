...
async function parseHttpError(
  response: Response,
): Promise<AgentCoreProjectedSseHttpError> {
  ...
  const body = await readBodyLimit(
    response,
  ...
  const details =
    error.details === undefined
      ? undefined
      : parsePublicJsonValue(error.details);
  return new AgentCoreProjectedSseHttpError(
    response.status,
    error.code,
    error.message,
    details,
...
function isEventStream(contentType: string | null): boolean {
  return (
    contentType !== null && /^text\/event-stream(?:\s*;|$)/iu.test(contentType)
  );
}
