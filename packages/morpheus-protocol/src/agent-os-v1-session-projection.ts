...
export const AGENT_OS_V1_SESSION_PROJECTION_SCHEMA =
  "agent-os-session-projection/v1" as const;
...
const AGENT_OS_V1_SESSION_PROJECTION_IDENTIFIER_PATTERN =
...
export type AgentOsV1SessionProjectionRequestBindingComparison =
  | "replay"
  | "independent";
...
export function parseAgentOsV1SessionProjectionReadRequest(
  ...
  if (value.schemaVersion !== AGENT_OS_V1_SESSION_PROJECTION_SCHEMA)
    fail("INPUT_INVALID");
  if (value.operation !== AGENT_OS_V1_SESSION_PROJECTION_OPERATION)
    fail("INPUT_INVALID");
...
function parseProjection(
  value: unknown,
  sessionId: string,
): Readonly<AgentOsV1SessionProjection> {
  const normalized = projectionUnsigned(value);
  if (normalized.projectionDigest === undefined) fail("INPUT_INVALID");
  if (
    normalized.projectionDigest !==
    projectionDigestFor(sessionId, normalized.projection)
  )
    fail("METADATA_CORRUPT");
  return deepFreeze({
    ...normalized.projection,
    projectionDigest: normalized.projectionDigest,
  });
}
...
export function createAgentOsV1SessionProjectionRequestBinding(input: {
  ...
  return deepFreeze({
    ...
    requestFingerprint: requestFingerprintFromParts(
      requestId,
      request.sessionId,
      request.cursor,
    ),
...
function projectionUnsigned(input: unknown): ProjectionParts {
  ...
  const hasDigest = Object.prototype.hasOwnProperty.call(
    value,
    "projectionDigest",
  );
  exact(
...
function isTurnStatus(
  value: unknown,
): value is AgentOsV1SessionProjectionTurnStatus {
  return (
    value === "preparing" ||
    value === "running" ||
    value === "waiting" ||
...
function copyValue(input: unknown, state: CopyState, depth: number): unknown {
  ...
  if (depth >= AGENT_OS_V1_SESSION_PROJECTION_LIMITS.maxDepth)
    fail("INPUT_INVALID");
  ...
  if (keys.length > AGENT_OS_V1_SESSION_PROJECTION_LIMITS.maxWidth)
    fail("INPUT_INVALID");
...
function countUtf8(value: string, state: CopyState): void {
  ...
  if (
    ...
    AGENT_OS_V1_SESSION_PROJECTION_LIMITS.maxTotalUtf8Bytes
  )
    fail("INPUT_INVALID");
}
