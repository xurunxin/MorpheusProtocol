...
export const AGENT_OS_V1_QUEUE_COMPACT_SCHEMA =
  "agent-os-queue-compact/v1" as const;
export const AGENT_OS_V1_PROMPT_QUEUE_READ_OPERATION =
  "prompt.queue.read" as const;
export const AGENT_OS_V1_PROMPT_QUEUE_CLEAR_OPERATION =
  "prompt.queue.clear" as const;
...
export type AgentOsV1QueueCompactRequestBindingComparison =
  | "replay"
  | "independent";
...
export function assertAgentOsV1PromptQueueLifecycleTransition(
  ...
  if (previousStatus === "queued") {
    if (nextStatus === "claimed" || nextStatus === "cancelled")
      return nextStatus;
    fail("STATE_CONFLICT");
  }
...
export function parseAgentOsV1PromptQueueReadResponse(
  ...
  if (disposition === "snapshot") {
    ...
    const cursor = opaqueText(
      value.cursor,
      AGENT_OS_V1_QUEUE_COMPACT_LIMITS.maxCursorUtf8Bytes,
    );
...
export function parseAgentOsV1SessionCompactReceipt(
  ...
  const requestFingerprint = digest(value.requestFingerprint);
  if (value.disposition !== "applied" && value.disposition !== "not_needed")
    fail("INPUT_INVALID");
  if (typeof value.replayed !== "boolean") fail("INPUT_INVALID");
  return deepFreeze({
    schemaVersion: AGENT_OS_V1_QUEUE_COMPACT_SCHEMA,
...
export function assertAgentOsV1QueueCompactResponseCorrelated(
  ...
  if (response.operation === AGENT_OS_V1_PROMPT_QUEUE_READ_OPERATION) {
    if (binding.operation !== AGENT_OS_V1_PROMPT_QUEUE_READ_OPERATION)
      fail("STATE_CONFLICT");
    if (!sameTarget(response.target, binding.target)) fail("STATE_CONFLICT");
    if (response.disposition === "not-modified") {
      if (binding.cursor === null || response.cursor !== binding.cursor)
        fail("STATE_CONFLICT");
    }
    return response;
  }
  if (response.operation === AGENT_OS_V1_PROMPT_QUEUE_CLEAR_OPERATION) {
    if (binding.operation !== AGENT_OS_V1_PROMPT_QUEUE_CLEAR_OPERATION)
      fail("STATE_CONFLICT");
    if (!sameTarget(response.target, binding.target)) fail("STATE_CONFLICT");
    if (
  ...
  if (binding.operation !== AGENT_OS_V1_SESSION_COMPACT_OPERATION)
    fail("STATE_CONFLICT");
  if (!sameTarget(response.target, binding.target)) fail("STATE_CONFLICT");
  if (response.requestFingerprint !== binding.requestFingerprint)
    fail("STATE_CONFLICT");
  return response;
}
  ...
  const request = async (
    ...
    try {
      return deepFreeze({
        ...
        response: assertAgentOsV1QueueCompactResponseCorrelated(
          response,
          binding,
        ),
      });
...
export const parseAgentOsV1QueueReadRequest =
  parseAgentOsV1PromptQueueReadRequest;
export const parseAgentOsV1QueueClearRequest =
...
function parseQueueItems(
  ...
  if (input.length > AGENT_OS_V1_QUEUE_COMPACT_LIMITS.maxItems)
    fail("INPUT_INVALID");
  ...
  for (const itemInput of input) {
    ...
    const itemId = identifierText(
      item.itemId,
      AGENT_OS_V1_QUEUE_COMPACT_LIMITS.maxItemIdUtf8Bytes,
    );
...
function parseQueueTarget(
  ...
  return deepFreeze({
    ...
    runId: identifierText(
      target.runId,
      AGENT_OS_V1_QUEUE_COMPACT_LIMITS.maxRunIdUtf8Bytes,
    ),
...
function parseRequestBinding(input: unknown): ParsedRequestBinding {
  ...
  if (operation === AGENT_OS_V1_PROMPT_QUEUE_READ_OPERATION) {
    ...
    const target = parseQueueTarget(value.target);
    const cursor =
      value.cursor === null ? null : opaqueText(value.cursor, 1_024);
    const requestFingerprint = digest(value.requestFingerprint);
...
type ParsedRequestBinding =
  | (AgentOsV1PromptQueueReadRequestBinding & {
      readonly fingerprintValid: boolean;
    })
  | (AgentOsV1PromptQueueClearRequestBinding & {
      readonly fingerprintValid: boolean;
    })
  | (AgentOsV1SessionCompactRequestBinding & {
      readonly fingerprintValid: boolean;
    });
...
function sameBindingIdentity(
  left: ParsedRequestBinding,
  right: ParsedRequestBinding,
): boolean {
  if (
  ...
  if (left.operation === AGENT_OS_V1_PROMPT_QUEUE_READ_OPERATION) {
    return (
      right.operation === AGENT_OS_V1_PROMPT_QUEUE_READ_OPERATION &&
...
function exactCommon(
  ...
  if (value.schemaVersion !== AGENT_OS_V1_QUEUE_COMPACT_SCHEMA)
    fail("INPUT_INVALID");
...
function queueFilter(value: unknown): AgentOsV1QueueClearFilter {
  if (value === "steer" || value === "follow_up" || value === "all")
    return value;
  fail("INPUT_INVALID");
}
...
function mapOwnerError(error: unknown): AgentOsV1QueueCompactContractError {
  if (isContractError(error))
    return new AgentOsV1QueueCompactContractError(error.code);
  return new AgentOsV1QueueCompactContractError("OWNER_UNAVAILABLE");
}

function isContractError(
  error: unknown,
): error is AgentOsV1QueueCompactContractError {
  return (
    typeof error === "object" && error !== null && CONTRACT_ERRORS.has(error)
  );
}
...
function copyValue(input: unknown, state: CopyState, depth: number): unknown {
  ...
  if (prototype !== Object.prototype || symbols.length !== 0)
    fail("INPUT_INVALID");
  const keys = Object.keys(descriptors);
  if (keys.length > AGENT_OS_V1_QUEUE_COMPACT_LIMITS.maxWidth)
    fail("INPUT_INVALID");
  const output: Record<string, unknown> = {};
  for (const key of keys) {
    countUtf8(key, state);
  ...
  try {
    const host = (
      globalThis as typeof globalThis & {
        readonly process?: BuiltinProcessHost;
      }
    ).process;
...
function isQueueTarget(
  value: AgentOsV1QueueCompactTarget,
): value is AgentOsV1PromptQueueTarget {
