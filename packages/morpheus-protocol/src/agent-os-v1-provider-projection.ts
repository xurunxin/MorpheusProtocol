...
export const AGENT_OS_V1_PROVIDER_PROJECTION_SCHEMA =
  "agent-os-provider-projection/v1" as const;
export const AGENT_OS_V1_PROVIDER_PROJECTION_OPERATION =
  "provider.read" as const;
export const AGENT_OS_V1_PROVIDER_READ_OPERATION =
...
const TRUSTED_TEXT_ENCODER_CONSTRUCTOR: typeof TextEncoder | undefined =
  typeof globalThis.TextEncoder === "function"
    ...
    : undefined;
const TRUSTED_TEXT_ENCODER_ENCODE:
  | ((this: TextEncoder, input: string) => Uint8Array)
  | undefined =
  TRUSTED_TEXT_ENCODER_CONSTRUCTOR === undefined
    ? undefined
    : findPropertyDescriptor(
        TRUSTED_TEXT_ENCODER_CONSTRUCTOR.prototype,
        "encode",
      )?.value;
...
export class AgentOsV1ProviderProjectionContractError extends Error {
  ...
  constructor(
    code: AgentOsV1ProviderProjectionContractErrorCode,
    _message = code,
  ) {
    const safeCode = isKnownContractErrorCode(code)
      ? code
      : "OWNER_UNAVAILABLE";
    super(safeCode);
    this.code = safeCode;
...
export type AgentOsV1ProviderProjectionSelectionState =
  | "selected"
  | "unselected"
  | "unknown";
...
export type AgentOsV1ProviderProjectionRequestBindingComparison =
  | "replay"
  | "independent";
...
export function parseAgentOsV1ProviderProjectionReadRequest(
  ...
  if (value.schemaVersion !== AGENT_OS_V1_PROVIDER_PROJECTION_SCHEMA)
    fail("INPUT_INVALID");
  if (value.operation !== AGENT_OS_V1_PROVIDER_PROJECTION_OPERATION)
    fail("INPUT_INVALID");
  const target = parseTarget(value.target);
  const cursor =
    value.cursor === null ? null : opaqueText(value.cursor, maxCursorBytes());
  return deepFreeze({
...
export function parseAgentOsV1ProviderProjectionReadResponse(
  ...
  if (value.disposition === "snapshot") {
    ...
    if (
      projectionDigest !==
      projectionDigestFor(target, projectionRevision, projection)
    )
...
export function createAgentOsV1ProviderProjectionRequestFingerprint(input: {
  ...
  return contentDigest(
    canonicalAgentOsV1ProviderProjectionRequestSource(input),
  );
}
...
export function assertAgentOsV1ProviderProjectionRequestBindingCompatible(
  ...
  if (!left.fingerprintValid || !right.fingerprintValid)
    fail("METADATA_CORRUPT");
  ...
  const request = async (
    ...
    try {
      return deepFreeze({
        ...
        response: assertAgentOsV1ProviderProjectionResponseCorrelated(
          response,
          binding,
        ),
      });
...
function parseTarget(
  input: unknown,
): Readonly<AgentOsV1ProviderProjectionTarget> {
...
function parseProjection(
  input: unknown,
): Readonly<AgentOsV1ProviderProjection> {
  const value = objectValue(input);
...
function captureReferenceClientOptions(
  ...
  if (typeof value.admit !== "function" || typeof value.read !== "function")
    fail("INPUT_INVALID");
  return {
...
function isContractError(
  error: unknown,
): error is AgentOsV1ProviderProjectionContractError {
  return (
...
function copyValue(input: unknown, state: CopyState, depth: number): unknown {
  ...
  if (isProxyObject(input)) fail("INPUT_INVALID");
  if (depth >= AGENT_OS_V1_PROVIDER_PROJECTION_LIMITS.maxDepth)
    fail("INPUT_INVALID");
  ...
  if (prototype !== Object.prototype || symbols.length !== 0)
    fail("INPUT_INVALID");
  const keys = Object.keys(descriptors);
  if (keys.length > AGENT_OS_V1_PROVIDER_PROJECTION_LIMITS.maxWidth)
    fail("INPUT_INVALID");
  const output: Record<string, unknown> = {};
  for (const key of keys) {
    countUtf8(key, state);
    ...
    const cleanup = (): void => {
      ...
      try {
        TRUSTED_REFLECT_APPLY(
          ABORT_SIGNAL_INTRINSICS.removeEventListener,
          signal,
          ["abort", onAbort],
        );
      } catch {
...
function countUtf8(value: string, state: CopyState): void {
  ...
  if (
    state.totalUtf8Bytes >
    AGENT_OS_V1_PROVIDER_PROJECTION_LIMITS.maxTotalUtf8Bytes
  )
    fail("INPUT_INVALID");
}
...
function utf8Length(value: string): number {
  if (
    TRUSTED_TEXT_ENCODER_CONSTRUCTOR === undefined ||
    TRUSTED_TEXT_ENCODER_ENCODE === undefined
  )
    fail("INPUT_INVALID");
  const encoder = new TRUSTED_TEXT_ENCODER_CONSTRUCTOR();
  const encoded = TRUSTED_REFLECT_APPLY(TRUSTED_TEXT_ENCODER_ENCODE, encoder, [
    value,
  ]);
...
const SHA256_CONSTANTS = [
  ...
  0x650a7354, 0x766a0abb, 0x81c2c92b, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
